...
| **先决条件** | [AEM 6.3 服务包 3 (6.3.3.0)](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) |
| **正式发布** | 2020 年 3 月 5 日 |
...
>[!NOTE]
>
>AEM Forms 附加组件包有助于使表单功能与 AEM 服务包和累积修订包保持一致。因此，请务必在安装任意 AEM 服务包、累积修订包或功能包之后再安装 AEM Forms 附加组件包。
...
* 在从“仅限内容”下拉列表中选择筛选器选项并选择移动选项，之前选定的资源（在触屏 UI 的列视图中）也会移动 (NPR-30693)。
...
* 用户关注左侧栏字段并使用键盘快捷键粘贴内容。在这样做时，它会粘贴页面编辑器剪贴板的内容，而不是从左边栏字段复制的内容（NPR-31173）。
* 当用户编辑内容片段时，会恢复内容片段的已删除变量 (NPR-31272)。
...
* AEM Author 环境中未填充分析报告。相反，会出现一个空白页（NPR-30905）。
...
* 使用名称中带有百分号 (%) 的文件夹解压缩存档文件时，无法通过 Assets 界面将其打开。NPR-29989：适用于 CQ-4270467 的修补程序
...
* 在生成 CSV 导出时，为列表视图添加自定义列会损坏报告。NPR-30208：适用于 CQ-4273344 的修补程序
* /etc/reports/ 中的 OOTB 报告无法正常工作，而且不显示历史数据图。NPR-30016：适用于 CQ-4220180 的修补程序
...
* 无法从 Asset 详细信息页面删除视频资源的图像呈现版本。NPR-29417：适用于 CQ-4268675 的修补程序
* 所有者无法在私有文件夹内创建私有文件夹。NPR-29397：适用于 CQ-4229830 的修补程序
* 上传超过 2 GB 的 Adobe Illustrator Artwork 文件会生成 Java™ 堆空间错误。NPR-29265：适用于 CQ-4226217 的修补程序
...
* 无论指定要搜索的路径是什么，路径字段都会在根路径打开。NPR-29398：适用于 CQ-4268897 的修补程序
...
* 在转出期间，空属性和多个属性不会从蓝图传播。使用 Blueprint 重置 Live Copy 对组件不起作用。NPR-29252：适用于 CQ-4264928、CQ-4264926、CQ-4267722 的修补程序
...
* 使用智能图像时，你无法将体验片段导出到目标。适用于 CQ-4269606 的修补程序

### 复制 {#replication}

* 复制代理组件容易受到漏洞攻击，该漏洞会向未经授权的用户泄露敏感信息。NPR-29613：适用于 Granite-25070 的修补程序
* 用户提供的数据不在 `cq/replication/components/agent` 组件的输出中进行转义，从而导致存储型跨站点脚本 (XSS) 漏洞。NPR-29452：适用于 CQ-4266263 的修补程序
...
* 无法从资源详细信息页面删除视频资源的图像演绎版本。NPR-29149：适用于 CQ-4266073 的修补程序
...
* 该文件 `coral.js` 包含该库的一个易受攻击的版本 `handlebars.js`。NPR-26973：适用于 CQ-4255377 的修补程序
* 如果将目标组件与嵌套式布局容器和文本组件一起使用，则在编辑文本或单击容器时，会引发以下 JavaScript 错误：“无法读取值为 null 的属性 currentPos”。NPR-29077：适用于 CQ-4246594 的修补程序
* （触屏 UI）无法将标记批量更新到已经用其他标记进行标记的页面。NPR-28729：适用于 CQ-4262922 的修补程序
* 在卡片视图中打开变量会导致 500 错误。NPR-28611：适用于 CQ-4263571 的修补程序
* 转出已在主版中移动的结构时会导致错误 `cq:moveTarget`。NPR-28968：适用于 CQ-4265280 的修补程序
...
* 共享的队列响应任务在 HTML5 工作区中打开一个闪光元素。NPR-29161：适用于 CQ-4266498 的修补程序
* 如果按钮包含 mlaut 字符，无法从工作区提交。NPR-29014：适用于 CQ-4263172 的修补程序

#### 表单 - 文档安全 {#forms-document-security}

* 在“策略集”视图页面上启用用于选择每页项目的选项。NPR-29243：适用于 CQ-4268567 和 CQ-4265132 的修补程序
...
* 搜索框中的特殊字符带有转义字符（\）前缀。NPR-27839：适用于 CQ-4259757 的修补程序
* 在快速搜索中搜索诸如 `(`、`+`、和`?` 之类的字符时出现错误。NPR-28212：适用于 CQ-4260969 的修补程序
* 无法使用 API 删除用户生成的内容中的评论。NPR-28075：适用于 CQ-4260534 的修补程序
* 当发布新评论时，发布到下一页的评论将以黄色高亮显示。NPR-28148：适用于 CQ-4259681 的修补程序
* 无法打开 Outlook 已发送邮件和收件箱文件夹中包含附件的邮件。NPR-28559：适用于 CQ-4217072 的修补程序
...
* 将内容框架加载到 editor.js 中后，列表`loaderPromises`不会被清除。NPR-27768：适用于 CQ-4205337 的修补程序
* 无法在嵌套 Parsys 上设置模板策略，除非在父组件上设置它。NPR-27987：适用于 CQ-4246095 的修补程序
...
* ResourceResolver 无法解析目标组件的多个域。NPR-28265：适用于 CQ-107847 的修补程序
* 如所显示的操作一样，LiveCopy 继承取消现在可以在目标容器上正常进行。NPR-28129：适用于 CQ-4259813 的修补程序
...
* 在文件夹元数据架构中，添加了对“规则”选项卡及其在资源文件夹上实施的支持。
...
* 通过链接共享电子邮件下载，会返回空的或损坏的 Zip 文件。NPR-27997：适用于 CQ-4259686 的修补程序
...
* 将核心组件表单与 Foundation Form 一起使用时出现空指针异常情况。NPR-27333：适用于 CQ-4249176 的修补程序
...
* 混合媒体集无法在查看器预设编辑器中播放视频。适用于 CQ-4251442 的修补程序
...
* 启用 Scorm 资源完成状态标签在 UI 中显示为空。NPR-27152：适用于 CQ-4249994 的修补程序
...
* 页面属性上的“保存并关闭”不会在 AEM WAR 中使用 Tomcat 返回正确的页面NPR-27567：适用于 Granite-23671 的修补程序
...
* 将 Form Builder 升级到最新的 Handlebars 版本。
...
* **“删除演绎版本”**&#x200B;选项不起作用，且引发错误“没有要删除的内容”。NPR-26007：适用于 CQ-4213414 的修补程序
...
* 复制使用已过期的令牌进行 OAuth 身份验证，导致多个错误。NPR-25894：适用于 GRANITE-22388 的修补程序
...
* 多个复制代理同时访问会导致访问访问令牌节点时日志出现异常。NPR-26964：适用于 GRANITE-23276 和 Granite-23155 的修补程序

#### ContextHub {#contexthub}

* 该文件 `coral.js` 包含该库的一个易受攻击的版本 `handlebars.js`。适用于 CQ-4255377 的修补程序
...
* 从 Adobe Storage Resource Provider 检索数据缓慢。NPR-26237：适用于 NPR-24152 的修补程序
...
* 论坛组件搜索存在问题。NPR-26577：适用于 CQ-4251303 的修补程序
...
* 无法创建或编辑文档安全的水印设置。NPR-26267、NPR-26129：适用于 CQ-4250234 的修补程序
...
* 编辑并保存可编辑模块，然后打开或关闭 PDF 预览，可保持信件的 PDF 和 HTML 预览都可见。两个预览在同一个窗口中保持功能正常。NPR-26770：适用于 CQ-4253217 的修补程序
...
* 设置 Date().getTime().toString 参数来初始化会话，而不是 Date.toString()。适用于 CQ-4241483 的修补程序
...
AEM 累积修订包 6.3.2.2 依赖于 AEM 6.3 服务包 2。因此，您必须先安装 AEM 6.3 服务包 2，然后再安装 AEM 累积修订包 6.3.2.x。有关安装说明，请参阅 [AEM 6.3 服务包 2](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) 中的发行说明。
...
* 下载大小与显示的信息不匹配。NPR-23418：适用于 CQ-4242774 的修补程序
...
* 预览后，Asset Admin Search 的 Facet 在搜索面板中消失。NPR-23964：适用于 CQ-4244410 的修补程序
...
* 无法在 Dynamic Media S7 (DMS7) 中注释单个演绎版本 MP4 视频资源。NPR-22046：适用于 CQ-4215912 的修补程序
* 对于 Tiff、金字塔生成流程，EmbedXMP 数据始终设置为“活动”。NPR-22903：适用于 CQ-4234498 的修补程序
* 图像预设计数较大时出现动态演绎版本显示/选择问题。NPR-23151：适用于 CQ-4217511 的修补程序
...
* 在将大约 80 个片段添加到资源中的文件夹后，从“时间轴”控制台触发工作流时会出现错误。NPR-23393：适用于 CQ-4211216 的修补程序
...
* （目标）使用 XML 等 API 类型时，不会激活广告系列。NPR-23199：适用于 CQ-4240936 的修补程序****
...
* DatePicker 不支持由隐藏字段设置的手动设置外部类型提示。更改类型提示会导致转换错误。NPR-23371：适用于 Granite-21194 的修补程序
...
* 未被标记的用户的标记原因仍然可见。NPR-23974：适用于 CQ-4243822 的修补程序
...
* 更新了 Sling Models 软件包，以包含字符集修补程序。
...
* 在“资源文件夹”设置中更改标题或缩略图时，文件夹的原始组和权限将被覆盖。NPR-22171：适用于 CQ-4216080 的修补程序
* UI 引发虚假错误消息“发布到 Brand Portal 时失败”，然而作业已添加到复制队列且资源已发布到 Brand Portal。NPR-22179：适用于 CQ-4205273 的修补程序
* （触屏 UI）列视图中资源的默认上传位置。NPR-22465：适用于 CQ-4237057 的修补程序
* 当尝试将资产架构从 `/conf/global`复制到`/conf/mytenant` 时，AEM 会抛出 StackOverflow 错误。NPR-22489：适用于 CQ-4235875 的修补程序
...
* 当有多个与 PDF 资源关联的关键字时，PDF 关键字元数据无法正确提取，并且会以错误方式修改。为解决该问题，已删除 PDF 资源的“主题”字段元数据属性。但是，您可以编辑元数据架构，为“主题”字段添加多值文本字段。NPR-21972：适用于 4215741 的修补程序****
...
* 在 Safari 上的资源中使用数字版权管理时，资源下载失败。用户无法下载带有免责声明和长文件名的资源。NPR-22747：适用于 CQ-4236460 和 CQ-4235274 的修补程序
* 将资源发布到 Brand Portal 时，将公开共享设置为默认选项。NPR-21931：适用于 CQ-4218816 的修补程序
...
* 在键入文本时，标签字段的筛选功能无法按预期发挥作用。NPR-22236：适用于 CQ-4236655 的修补程序
...
* 当 ContextHub 存储在初始化期间覆盖默认持久层时，会出现持久性问题。NPR-22479：适用于 CQ-4218399 的修补程序
* 若未在第一个内容根上选中“包含子页面”，则使用多个页面启动不会将子页面发布到发布服务器。NPR-22482：适用于 CQ-4237818 的修补程序
* （Touch UI）通过经典 UI 控制台删除启动项会使所有页面无法编辑。NPR-22491：适用于 CQ-4225074 的修补程序
* 由于对话框中有多余空间，图像组件出现问题。NPR-22528：适用于 CQ-4238183 的修补程序
* 当使用内联模式打开组件时，之前加载的插件再次不可见。NPR-22591：适用于 CQ-4236850 的修补程序
...
* Sling Exporter servlet 无法正确导出日语字符。NPR-22153：适用于 CQ-4228920 的修补程序
...
* 目标引擎（mbox.js、at.js）不使用损坏的 URL。并且，使用包含冒号的 URL 可能会导致某些部署失败。NPR-22366：适用于 CQ-4237854 的修补程序
...
* 为了提高安全性，CAPTCHA 组件现已弃用。如果您使用 CAPTCHA 组件，安装 6.3.2.1 SP2-CFP15 或更高版本后会显示消息“Captcha 组件已弃用，不应使用”。可以自定义 AEM 组件来包含 reCAPTCHA 以提高安全性。NPR-22151：适用于 CQ-4220052 的修补程序

### WCM - 页面编辑器 {#wcm-page-editor}

* 使用无效选择器时会出现反射型跨站点脚本 (XSS)。适用于 CQ-4270397 的修补程序
...
* （经典 UI）即使关联的 Form Data Model 服务设置为空字段，组件也会显示默认值。NPR-22272：适用于 GRANITE-19744 的修补程序
...
* 检查 ECM Connectors Configuration 页面的密码字段时，密码以纯文本形式显示。NPR-22508：适用于 CQ-4236065 的修补程序
...
* 规则编辑器的自定义函数在函数调用后添加了一个额外的。因此，即使自定义函数返回 true，它们仍会验证失败。NPR-22481：适用于 CQ-4235499 的修补程序
...
* (Microsoft® Edge) (IE11) 具有多行字段的自适应表单文本字段组件显示 `Null` 作为默认值，而不是空白。NPR-22284：适用于 CQ-69107 的修补程序
* 自适应表单中的 SOAP UTF-8 输入编码会返回页面乱码错误。NPR-20105：适用于 CQ-4222669 的修补程序
* 如果在 AEM Sites 页面中配置了错误的表单，AEM Forms 容器组件将不可编辑。适用于 CQ-4237456 的修补程序
...
* 发布页面后修改的表单不会再次发布。NPR-23013：适用于 CQ-4236566 的修补程序
...
* （HTML 工作区）关闭工作区中的表单时会触发“准备数据”。NPR-21127：适用于 CQ-4224574 的修补程序
...
* 通过 HSM 对 PDF 文档进行数字签名时遇到异常。NPR-21154：适用于 CQ-4226978 的修补程序
...
* 无法从许可证检查页面下载带有空格的资源。NPR-20067：适用于 CQ-4216557 的修补程序
* 上传具有多个 Alpha 层的 PSB 文件时出现处理问题。NPR-20250：适用于 CQ-4220869 的修补程序
* 用户无法下载定义了长文件名和免责声明的资源。NPR-20254
...
* DAM 资源 API 元数据缺陷导致 xmp 回写 API 受损。NPR-20607：适用于 CQ-4220455 的修补程序
...
* AEM Dynamic Media Classic（以前称为 Scene7）无法从 AEM 服务器同步资产。当 Cloud Service Configuration 中的目标路径指向根路径子文件夹时，就会出现此问题。CQ-4228265
...
* Search&amp;Promote cloud service 配置忽略表单检索方法，导致其在创作实例上不可用。NPR-20447：适用于 CQ-4206076 的修补程序
* 安装 Search&amp;Promote 功能后，内容包中模糊不清的筛选器定义导致路径被覆盖。NPR-20808

### Mobile On-Demand {#mobile-on-demand}

* 每次显示属性时，TXT 类型资源的元数据属性会显示在不同的元数据编辑器中。NPR-20239
...
* 其他 data-sly-resource 语句的 requestAttributes 目前尚未设置。NPR-20639：适用于 4226206 的修补程序
...
* 尽管对话框中存在属性，但颜色字段仍设置为“始终必需”。NPR-19702
...
* 即使关联的 Form Data Model 服务未返回值，下拉列表也会显示上次存储的值。NPR-20710
...
* 无法将开关组件的值设置为 true 或 false。NPR-21111
...
* 当图像的 dc: title 属性设置为 `String [] (multifield)` 时，图像卡上显示标题的行为发生变化
* 使用 Foundation 时间组件替换了服务器端时间呈现。
* 启用通过 tagadmin/tagging 控制台将标记从 AEM 发布到 Brand Portal 功能。
* 已发布 JSON API 以使用内容片段。
...
* 无法使用 Chrome 在列视图中加载完整的页面/资源列表。NPR-19566：适用于 CQ-4214248 的修补程序
...
* 已发布 JSON API 以使用内容片段。NPR-19500
...
* 项目编辑者无法将资源复制和粘贴到项目资源文件夹中。NPR-19619：适用于 CQ-4215321 的修补程序
...
* 如果使用 Google Chrome 浏览器，用户无法通过触屏 UI 中的列视图来加载页面/资源的完整列表。NPR-19566：适用于 CQ-4214248 的修补程序

### Brand Portal {#brand-portal-1}

* 无法通过 AEM 发布带有评论和注释的资源。NPR-19590：适用于 CQ-4218386 的修补程序
* 启用通过 tagadmin/tagging 控制台将标记从 AEM 发布到 Brand Portal 功能。NPR-20271：适用于 CQ-4223948 的修补程序
...
* 在需要为外部连接配置 org.apache.http.proxyconfigurator 的环境中使用 Cloud Service 配置 Google `reCaptcha` 时，POST 调用似乎无法穿过代理。NPR-20454
* 在区域设置包含除“.”之外的小数点分隔符的安装中，基于 XSD 架构的自适应 Forms 会为数字字段提交错误的 XML 值，从而导致出现错误。NPR-20444
...
* 无法将下拉式元数据架构字段设为必填字段，因为资源缺少必填字段验证。NPR-17882：适用于 CQ-4208373 的修订包
...
* 日语文件夹下的资源审核任务无法正常执行。CQ-4204782
* 从资源的属性页面移动资源时，会发生空指针异常。CQ-4204251
* 如果资源多次链接到 InDesign 文档，则 AEM 无法在属性页面中跟踪对资源的后续引用。CQ-4204186
* 在混合设备上的 Chrome 中编辑元数据架构表单后，在元数据架构表单中添加新选项卡时出现问题。CQ-4201810
* 上传重复资源时，即使在“检测到重复项”对话框中未选中（删除/保留）选项，相应的选项也会应用于所有资源。CQ-4201673
...
* 在 Live Copy 中重新应用继承时，修改后的本地化组件无法恢复为原始形式。NPR-18172：适用于 CQ-4211379 的修补程序
...
* 修复了通信管理、Document Security 和 Forms Workflow 的可用性问题。
...
* 项目符号列表上方和下方的额外边距在文本编辑器中可见，但在 HTML 和 PDF 演绎版中不可见。NPR-18126
...
* PDF Generator 服务无法将大于 100 MB 的图像转换为 PDF 文档。CQ-4208628
* 将 PDF Generator 服务与日语 OCR 一起使用时，会生成倒置的 PDF。NPR-17602
...
>[!NOTE]
>
>Adobe 建议先对每个新的安装包运行验证，然后再尝试安装。预验证会分析并报告安装之前发现的任何错误，并主动向用户发出有关此类错误的警告。
>
>您可以在[https://experienceleague.adobe.com/zh-hans/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)访问验证选项文档

* AEM 6.3.3.0 是安装 CFP 的先决条件。请参阅[升级文档](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)，了解关于将 AEM 安装升级至 AEM 6.3 的详细说明。
...
在 crx -quickstart /logs 文件夹中会创建 activity.log。
...
>[!NOTE]
>
>如果要安装 6.3.3.0 或更高版本，请执行以下步骤来配置 JBoss® 应用程序服务器的设置。如果是在 Oracle WebLogic 或 IBM® WebSpehere 应用程序服务器上运行的 AEM Forms 服务器上安装 6.3.3.0，则无需其他配置。有关更多详细信息，请参阅 [AEM 6.3.3.0 发行说明](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)。
...
* 由于管理员搜索功能中的请求过多，观察到 CPU 使用率很高。NPR-24229
...
默认值为 1。通过覆盖文件 constants.js（`/libs/cq/ui/widgets/source/constants.js`）并取消注释 `CQ.PAGE_PROPERTIES_MAX_RECURSION_LEVEL` 属性来覆盖该值。然后，为其分配所需的值（页面 `/jcr:content`下的最大深度，存储页面属性的数据）。
...
此过程使 sling GET servlet 能够返回所有所需的节点。

## Uber Jar {#uber-jar}

适用于 6.3.3.8 的 Uber Jar 可在 Adobe 公共 Maven 存储库中获取。

要在 Maven 项目中使用 Uber Jar，请参阅[如何使用 Uber Jar](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) 一文，并在项目 POM 中包含以下依赖项：
...
>[!MORELIKETHIS]
>
>* [AEM 发行版和更新](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-release-information/aem-release-updates/aem-releases-updates)
>* [AEM 6.3 修补程序页面](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-release-information/aem-release-updates/aem-releases-updates)
>* [AEM 6.3 发行说明](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)
>* [AEM 产品页面](https://business.adobe.com/products/experience-manager/adobe-experience-manager.html)
>* [AEM 6.3 文档](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)
