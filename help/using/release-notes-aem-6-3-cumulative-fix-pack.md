...
* 在“资源文件夹”设置中更改标题或缩略图图片时，文件夹的原始组和权限会被覆盖。NPR-22171：适用于 CQ-4216080 的修补程序
...
* AEM 在尝试将资源模式从 /conf/global 复制到 /conf/ mytenant 时抛出 StackOverflow 错误。NPR-22489：适用于 CQ-4235875 的修补程序
...
* 在 Safari 上的资源中使用数字版权管理时，资源下载失败。用户无法下载包含免责声明和长文件名的资源。NPR-22747：适用于 CQ-4236460 和 CQ-4235274 的修补程序
...
* 在调用函数后，规则编辑器的自定义函数添加一个额外的“;”，因此，即使自定义函数返回 true，验证也会失败。NPR-22481：适用于 CQ-4235499 的修补程序
...
* 当云服务配置中的目标路径指向根路径中的子文件夹时，AEM Dynamic Media Classic（以前的 Scene7）无法从 AEM 服务器同步资源。CQ-4228265
...
* 无法使用 Chrome 在列视图中加载页面/资源的完整列表。NPR-19566：适用于 CQ-4214248 的修补程序
...
* 项目编辑者无法将资源复制/粘贴到项目资源文件夹中。NPR-19619：适用于 CQ-4215321 的修补程序
...
* 修复了通信管理、文档安全和 Forms Workflow 的可用性问题。
...
* PDF Generator 服务无法将大于 100 MB 的图像转换为 PDF 文档。Ref# CQ-4208628
...
>
>您可以访问相关文档以了解“验证”选项，网址为：[https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)

* AEM 6.3.3.0 是安装 CFP 的先决条件。请访问[升级文档](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)，了解关于将 AEM 安装升级至 AEM 6.3 的详细说明。
...
>[!NOTE]
>
>如果要安装 6.3.3.0 或更高版本，请执行以下步骤来配置 JBoss® 应用程序服务器的设置。如果是在 Oracle WebLogic 或 IBM® WebSpehere 应用程序服务器上运行的 AEM Forms 服务器上安装 6.3.3.0，则无需其他配置。有关更多详细信息，请参阅 [AEM 6.3.3.0 发行说明](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)。
...
默认值为 1。覆盖该值，方法为：覆盖文件 constants.js (`/libs/cq/ui/widgets/source/constants.js`)，取消对 CQ.PAGE_PROPERTIES_MAX_RECURSION_LEVEL 属性的注释，并为其指定所需值（页面的 `/jcr:content` 下的最大深度，在达到该深度之前，将存储页面属性数据）。
