...
| **先决条件** | [AEM 6.3 Service Pack 3 (6.3.3.0)](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) |
| **正式发布** | 2020年3月5日 |
...
AEM累积修补程序包6.3.3.8依赖于AEM 6.3 Service Pack 3。 因此，您必须先安装 AEM 6.3 Service Pack 3，然后再安装 AEM 累积修补程序包 6.3.3.x。有关安装说明，请参阅 [AEM 6.3 Service Pack 3](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions).
...
>[!NOTE]
>
>对于版本低于6.3.3.0的AEM实例，Adobe建议通过install文件夹的方式部署SP/CFP，以用于在AEM实例上拥有多个用户的客户。
...
* 选择一个Parsys及其中包含的某个组件并使用键盘快捷键删除选定项目时，此操作会同时删除该组件及其父Parsys (NPR-32071)。
...
* 在创作实例中更改并激活的组件内容在重新启动该组件之前不会显示在发布实例中 **com.day.cq.personalization.impl.TargetedContentManagerImpl** (NPR-32489和NPR-32232)。
* 上下文中心性能在发布时崩溃(NPR-31170)。

### Brand Portal {#brand-portal}

* Adobe Developer未与适用于Brand Portal的Adobe Experience Manager 6.3集成(NPR-32056)。
...
* （仅限JEE）针对Apache报告的严重安全漏洞(CVE-2021-44228和CVE-2021-45046) `Log4j2`.
...
AEM累积修补程序包6.3.3.7依赖于AEM 6.3 Service Pack 3。 因此，您必须先安装 AEM 6.3 Service Pack 3，然后再安装 AEM 累积修补程序包 6.3.3.x。有关安装说明，请参阅 [AEM 6.3 Service Pack 3](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions).
...
* 主页面的页面属性加载失败，并返回NullPointerException。 添加的问题得以解决 `cq:blueprin`t属性(NPR-30901)。
* 无法从根节点上的 blueprintConfig 正确检索到转出配置。触发了 Blueprint 和 Live Copy 的停用。仅触发Blueprint的停用(NPR-30866)。
* 用户转出页面时，转出配置对话框显示重复的Live Copy路径(NPR-30438)。
* 开箱即用的基架富文本编辑器 (RTE) 意外地将内嵌字体大小应用到元素（NPR-31283、NPR-30922）。
* 无法同步Adobe Campaign中包含现成设计导入程序组件的营销活动(NPR-30890)。
* 富文本编辑器(RTE)不允许您将嵌入式表作为列表项插入(NPR-30878)。
* 如果用户的焦点位于左侧边栏字段并使用键盘快捷方式粘贴内容，则会粘贴页面编辑器剪贴板的内容，而不是从左侧边栏字段复制的内容 (NPR-31173)。
* 用户编辑内容片段时，会恢复内容片段的已删除变量 (NPR-31272)。
* AEM站点不创建语言副本(NPR-30690)。
* 页面编辑器的操作没有转出活动副本的控件(NPR-30613)。
...
AEM累积修补程序包6.3.3.6依赖于AEM 6.3 Service Pack 3。 因此，您必须先安装 AEM 6.3 Service Pack 3，然后再安装 AEM 累积修补程序包 6.3.3.x。有关安装说明，请参阅 [AEM 6.3 Service Pack 3](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions).

### 资产 {#assets-2}

* Dynamic Video 的视频整合仅返回结果集中的前 100 个条目。NPR-30441：适用于 CQ-4213561 的修补程序
* 通过Data powerAdobe智能标记连接问题。 NPR-30026：适用于 CQ-4269457 的修补程序
...
#### Forms - Acrobat服务 {#forms-document-services}
...
AEM累积修补程序包6.3.3.5依赖于AEM 6.3 Service Pack 3。 因此，您必须先安装 AEM 6.3 Service Pack 3，然后再安装 AEM 累积修补程序包 6.3.3.x。有关安装说明，请参阅 [AEM 6.3 Service Pack 3](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions).
...
* 上载超过2 GB的Adobe Illustrator图稿文件会生成Java™栈空间错误。 NPR-29265：适用于 CQ-4226217 的修补程序
...
* 在转出期间，不会从 Blueprint 传播空属性和多个属性。通过Blueprint重置Live Copy对组件不起作用。 NPR-29252：适用于 CQ-4264928、CQ-4264926、CQ-4267722 的修补程序
...
AEM累积修补程序包6.3.3.4依赖于AEM 6.3 Service Pack 3。 因此，您必须先安装 AEM 6.3 Service Pack 3，然后再安装 AEM 累积修补程序包 6.3.3.x。有关安装说明，请参阅 [AEM 6.3 Service Pack 3](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions).
...
* 如果重新上传具有相同名称的存档，则不会为新处理的资源生成演绎版。 NPR-28643：适用于 CQ-4262286 的修补程序
...
* 上载大型zip存档资产时会触发CommitFailedException。 NPR-28528：适用于 CQ-4260903 的修补程序
...
* 如果将Target组件与嵌套式布局容器和文本组件一起使用，则在编辑文本或单击容器时，会引发以下JavaScript错误：“无法读取null的currentPos属性”。 NPR-29077：适用于 CQ-4246594 的修补程序
* （触屏 UI）无法将标记批量更新到已使用其他标记进行标记的页面。NPR-28729：适用于 CQ-4262922 的修补程序
* 在卡片视图中打开变量会导致 500 错误。NPR-28611：适用于 CQ-4263571 的修补程序
* 转出已在主节点中移动的结构会导致错误 `cq:moveTarget`. NPR-28968：适用于 CQ-4265280 的修补程序

### 集成 {#integration}

* (Cloud Service配置)在根级别显示的“继承自”复选框应被删除。 NPR-28771：适用于 CQ-4259676 的修补程序
* com.day.cq.personalization.impl.TeaserResourceEventHandler 陷入无限循环，并导致对发布实例上的节点进行更新。NPR-28561：适用于 CQ-4263096 的修补程序
* 使用Bright Edge凭据失败，出现连接错误。 NPR-29167：适用于 CQ-4265872 的修补程序
...
* 单击标记会删除全局鼠标悬停事件，并且对话框在“可拖动模式”下冻结。 NPR-28641：适用于 CUI-7294 的修补程序
...
* “共享队列响应”任务会在HTML5工作区中打开一个Flash元素。 NPR-29161：适用于 CQ-4266498 的修补程序
...
#### Forms - Acrobat服务 {#forms-document-services-1}
...
AEM累积修补程序包6.3.3.3依赖于AEM 6.3 Service Pack 3。 因此，您必须先安装 AEM 6.3 Service Pack 3，然后再安装 AEM 累积修补程序包 6.3.3.x。有关安装说明，请参阅 [AEM 6.3 Service Pack 3](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions).
...
* 无法下载多个文件类型 `.jp2`， `.max`， `.oft`， `.msg`. NPR-28002：适用于 CQ-4210856 的修补程序
...
* 搜索类似这样的字符时出错 `(` ， `+` ， `?` 进行快速搜索。 NPR-28212：适用于 CQ-4260969 的修补程序
...
* 若未设置父组件，则无法在嵌套的Parsys上设置模板策略。 NPR-27987：适用于 CQ-4246095 的修补程序
* 组件浏览器未清理用户输入，因此可能会引发JavaScript错误。 NPR-27986：适用于 CQ-4247590 的修补程序
* 用户尝试编辑内容片段时，页面显示为空白。NPR-27669
* 用户单击注释时，注释高亮显示。 BPR-27196：适用于 CQ-4254423 的修补程序
...
* 复制被6.3.3.1中的DispatcherFlushRules破坏。NPR-28150：适用于CQ-4261401的修补程序
...
* 通过链接共享电子邮件下载会返回空/损坏的zip。 适用于 CQ-4259686 的修补程序
...
* 将MS® Translator升级到API v3.0后，在AEM 6.3中启用对MS® Translator服务的支持。NPR-28365：适用于CQ-4259096的修补程序
...
#### Forms - Acrobat服务 {#forms-document-services-2}

* 在Admin Console的“策略集”视图中，无法看到除列出的前1000项之外的任何策略集。 NPR-28060、NPR-26047：适用于 CQ-4249865 的修补程序
* 引发名为的异常 `java.lang.IllegalArgumentException message:No enum constant com.adobe.internal.pdfm.docbuilder.signature.PathValidationFailureReason.SIGNED_IN_FUTURE` 无法完成短暂的进程。 NPR-28652
...
AEM累积修补程序包6.3.3.2依赖于AEM 6.3 Service Pack 3。 因此，您必须先安装 AEM 6.3 Service Pack 3，然后再安装 AEM 累积修补程序包 6.3.3.x。有关安装说明，请参阅AEM 6.3 Service Pack 3发行说明。
...
* 动态下拉列表不支持层叠下拉列表。 NPR-27044：适用于 CQ-4252564 的修补程序
* 改进了查询，以便使用ExpiryNotification功能。 NPR-26999：适用于 CQ-4251188 的修补程序
...
* （经典UI）选择时出现性能问题，如果存在多个下拉列表，则更改了侦听器。 NPR-27115：适用于 CQ-4237215 的修补程序
...
* 此 `cq:actions` 目标组件不考虑使用。 NPR-27616：适用于 CQ-4257497 的修补程序
...
* 确认删除后单击图像上的垃圾桶图标会永久移除附件。 NPR-27340：适用于 CQ-4255150 的修补程序
...
* 在带有Tomcat的AEM WAR中，“保存并关闭”页面属性不会返回到正确的页面。 NPR-27567：适用于 Granite-23671 的修补程序
...
#### Forms - Acrobat服务 {#forms-document-services-3}

* 在WebSphere®上基于SOAP创建表单数据模型失败。 NPR-27692：适用于 CQ-4253702 的修补程序
...
AEM累积修补程序包6.3.3.1依赖于AEM 6.3 Service Pack 3。 因此，您必须先安装 AEM 6.3 Service Pack 3，然后再安装 AEM 累积修补程序包 6.3.3.x。有关安装说明，请参阅中的发行说明。 [AEM 6.3 Service Pack 3](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions).
...
* 在资产上创建许多关系会导致错误。 NPR-26309：适用于 CQ-4250708 的修补程序
...
* 经典UI中子规则的区段规则(/etc/segmentation)激活导致发布下降。 NPR-26601：适用于 CQ-4253588 的修补程序
* 添加“特殊字符”会将“富文本编辑器”对话框滚动到顶部。 NPR-26435：适用于 CQ-4249869 的修补程序
* （触屏UI）从全屏切换到浮动对话框时，多个富文本编辑器的工具栏变得不可用。 NPR-25652：适用于 CQ-4206008 的修补程序
...
* 将变量创建为Live Copy需要用户刷新页面才能反映。 NPR-26663：适用于 CQ-4254328 的修补程序
* （经典 UI）返回页面属性后，缩略图图像不再使用继承并从站点管理和 Sidekick 中消失，导致图像显示为空白。NPR-26562：适用于 CQ-4252346 的修补程序
* 在创建页面版本并触发比较时，/content/versionshistory中的节点将列在Blueprint的实时副本列表中。 NPR-26506：适用于 CQ-4243957 的修补程序
...
* 发布包含新资产的页面时，会在不发出通知的情况下停用发布实例中的子页面。 NPR-24869：适用于 CQ-4247832 的修补程序
* 复制使用过期的OAauth令牌。 NPR-25984：适用于 Granite-22388 的修补程序

#### 复制 {#replication-2}

* Http 传输可能会泄漏处于打开状态的会话。适用于 Granite-17434 的修补程序
* 多个复制代理同时访问访问令牌节点时，日志中出现异常。 NPR-26964：适用于 GRANITE-23276 和 Granite-23155 的修补程序
...
* 私人消息合成页面不稳定且缓慢。 NPR-26120：适用于 CQ-4250923 的修补程序
...
* 审查方取消固定内容且无特色内容不适用于MySQL单责任原则(SRP)。 NPR-26767：适用于 CQ-4251520 的修补程序
...
* 在社区消息中同时搜索名字和姓氏不会返回预期结果。 NPR-26377：适用于 CQ-4253150 的修补程序
...
* 如果组包含数千用户，则不会加载组设置。 NPR-25642：适用于 CQ-4238426 的修补程序
* 目录资源播放器无法使用上下文路径。NPR-25640：适用于 CQ-4238426 的修补程序
* （论坛搜索）- 包含大量帖子的线程的分页链接不适用于通知。适用于 CQ-4254202 的修补程序
* 使用Firefox时，审核控制台仅显示五个条目。 适用于 CQ-4254128 的修补程序
...
* 更新granite coralui2和coralui3 clientlibs以从内部版本中删除Handlebars。 NPR-25606：适用于 Granite-22116 的修补程序
...
#### Forms - Acrobat服务 {#forms-document-services-4}
...
AEM累积修补程序包6.3.2.2依赖于AEM 6.3 Service Pack 2。 因此，您必须先安装 AEM 6.3 Service Pack 2，然后再安装 AEM 累积修补程序包 6.3.2.x。有关安装说明，请参阅中的发行说明。 [AEM 6.3 Service Pack 2](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions).
...
* 为文件扩展名LSR和SKETCH提取的MIME类型不正确，并导致文件下载无效。 NPR-23644：适用于 CQ-4243260 的修补程序
...
* (Brand Portal)在发布资源时允许创建子资源。 NPR-23331：适用于 CQ-4242018 的修补程序
...
* 对于Tiff（金字塔生成流程），EmbedXMP数据始终设置为“活动”。 NPR-22903：适用于 CQ-4234498 的修补程序
...
* 扩展对话框时，页面属性中的“缩略图”选项卡不起作用。 NPR-22844：适用于 CQ-4241474 的修补程序
...
* 无法保存带有移动设备/平板电脑版本中必需选择下拉列表的对话框。 NPR-23904：适用于 CQ-4243096 的修补程序
...
* 通过Dispatcher访问并为其启用Sling Dynamic Include时，Communities组件出现问题。 NPR-23586：适用于 CQ-4242360/CQ-4241522 的修补程序
...
* 尽管选择了默认排序顺序，排序按钮文本仍不显示默认选定值。 NPR-23881：适用于 CQ-4243375 的修补程序
...
* 转出页面时，Live Copy不显示从主页面继承的打开/关闭时间属性。 NPR-23873：适用于 CQ-4243431 的修补程序
...
* 移动 `ReCaptchaConfigService` 到内部包。 适用于 CQ-4217459 的修补程序
* 数字字段不符合最小值。NPR-23967：适用于 CQ-4244830 的修补程序
* 自适应Forms与Adobe Sign集成中支持多分片。 NPR-23383
...
* 如果在显示两个弹出窗口之间更改了所选文件夹，则会删除错误的资源。 NPR-21980：适用于 CQ-4233675 的修补程序
...
* （经典UI）富文本编辑器输出html将“list-position-style： inside；”作为内嵌样式添加到 &lt;ul> 标记之前。 NPR-22145：适用于 CRTE-114 的修补程序
...
* 在键入文本时，标记字段的筛选功能无法按预期发挥作用。 NPR-22236：适用于 CQ-4236655 的修补程序
* （页面编辑器）在图像映射组件中粘贴文本数据时，也会粘贴文本组件。NPR-22264：适用于 CQ-4236230 的修补程序
* 对话框文件上载字段必填导致对话框提交时出现问题。 NPR-22464：适用于 CQ-4222192 的修补程序
...
* 如果未在第一个内容根上选中“包含子页面”，则包含多个页面的启动项不会将子页面发布到发布服务器。 NPR-22482：适用于 CQ-4237818 的修补程序
...
* 删除嵌套启动项中的启动项会导致子启动项被孤立。 NPR-22621：适用于 CQ-4202639 的修补程序
* （经典 UI Sidekick）当页面处于工作流锁定阶段时，“工作流”选项卡被禁用。NPR-22722：适用于 CQ-4237557 的修补程序
* 在页面上翻转添加到图像组件中的图像后，更改未保存，并且原始图像显示在页面上。 呈现支持已通过以下方式添加到图像核心组件 [https://github.com/adobe/aem-core-wcm-components/pull/141](https://github.com/adobe/aem-core-wcm-components/pull/141). NPR-22801：适用于 CQ-4221539 的修补程序
...
* 操作系统剪贴板和内部AEM剪贴板的触屏UI中存在复制/粘贴问题。 NPR-22807：适用于 CQ-4220383 的修补程序
...
* 由于缺少，PersonalizationDataSource引发空指针异常 `jcr:content` 节点。 NPR-22850：适用于 CQ-4222122 的修补程序
...
* 内存泄漏导致资产共享AEM发布器实例出现稳定性问题。 NPR-22205、NPR-23178：适用于 Sling-5668、Sling-7292 和 Sling-7470 的修补程序。
* 不应使用不稳定的会话属性名称服务ID。 NPR-22821：适用于 Granite-21059 的修补程序
* 当http白板管理的会话失效时，如果容器会话不含其他会话属性，则容器会话也将失效。 NPR-23059：适用于 FELIX-5819 的修补程序
...
* 在调用函数后，规则编辑器的自定义函数添加一个额外的；，即使自定义函数返回true，验证也会失败。 NPR-22481：适用于 CQ-4235499 的修补程序
...
* 一旦在AEM Forms页面中配置了错误的表单，AEM Sites容器组件便无法进行编辑。 适用于 CQ-4237456 的修补程序
...
* 关于PDF/A-1b合规性检查错误的Acrobat DC报告和AEM报告之间存在差异。 NPR-22051、NPR-22050：适用于 CQ-4226128、CQ-4227671 的修补程序
...
* PDF Generator无法生成具有指定书签级别的PDF文档。 NPR-22921、NPR-22285：适用于 CQ-4230562 的修补程序
...
* 无法正确下载包含名称与UTF-8不兼容的资源的压缩文件。 NPR-20420：适用于 CQ-4219961 的修补程序
...
* 在富文本编辑器中的两行上应用样式化文本，然后将它们合并到一个段落中，会删除已设置样式的范围。 NPR-20110：适用于 CQ-4223051 的修补程序
...
* 占位符在包含多个Parsys组件的页面中不可见。 NPR-20436：适用于 CQ-4215000 的修补程序
...
* 搜索结果组件与搜索结果不一致。 NPR-20070：适用于 CQ-4220913 的修补程序
...
* 安装AEM 6.3.0.1会删除 `dam-update-service` 用户的密钥库。 NPR-20018
* 在某些自定义部署中，用户尝试在addTask模块中选择被分配人时，需要较长时间才能填充用户选取器中的列表。 NPR-20283：适用于 CQ-4224193 的修补程序

### 用户界面 {#user-interface-3}

* 尽管对话框中存在属性，颜色字段仍设置为“始终必需”。 NPR-19702
* 在Internet Explorer 11上，组件多字段的全屏模式中不显示滚动条。 NPR-20261：适用于 CQ-4219782 的修补程序
* 如果触发连续查询导致不正确的结果，之前的查询不会被中止。NPR-20398：适用于 GRANITE-19306 的修补程序

### 工作流 {#workflow-1}

* 用户在其收件箱中收到工作流任务时，不会收到相关通知。 NPR-20213：适用于 CQ-4221639 的修补程序
* 在单击工作流模型的对话框参与者步骤中的下拉列表时，OOTB Granite用户选取器未加载任何用户。 NPR-20236
...
* 即使关联的表单数据模型服务未返回值，下拉菜单也会显示上次存储的值。 NPR-20710
...
* dc： title和dc： description在CRXDE Lite中不会更改为多字段值。 NPR-19570：适用于 CQ-4209086 的修补程序
...
* 将实例升级到AEM 6.1SP2-CFP3后，由于Sidekick.js中的代码发生更改，多复合字段下拉值未重新填充。 NPR-19450：适用于 CQ-4194771 的修补程序
* WCMMode.EDIT 在创作模式下不适用于目标组件。NPR-19387
* 发布了 JSON API 以使用内容片段。NPR-19500
* 在“创作”对话框中，粗体、斜体、下划线功能不适用于富文本编辑器字段。 NPR-19670、NPR-19718：适用于 CQ-4219088 的修补程序
...
* 修复Brand Portal云服务配置UI上的“已启用”字段。 适用于 CQ-4211101 的修补程序
...
* 在Safari中，自适应Forms不允许将PDF作为附件。 NPR-19625
* RestoreGuideState 会覆盖 customcontextproperty 映射。CQ-4222877
* 配置Google时 `reCaptcha` 使用Cloud Service时，在需要配置org.apache.http.proxyconfigurator以进行外部连接的环境中，POST调用似乎不会通过PROXY。 NPR-20454
...
* 将表单最大化/最小化并将其另存为草稿或转发后，工作流附件、注释和详细信息选项卡在工作区中无法正常工作。 NPR-20243
...
* 当在JBoss®应用程序的群集配置中运行的AEM Forms Server停止时，该应用程序服务器将与数据库断开连接。 这可能导致数据损坏问题。NPR-19724

### 包含的功能包 {#feature-packs-included-1}

* 由于资产缺少必填字段验证，因此无法将下拉元数据架构字段设为必填字段。 NPR-17882：适用于 CQ-4208373 的修补程序包
...
* 上传了重复的资源时，即使未在检测到的重复项对话框中选择该选项，（删除/保留）选项也将应用于所有资源。 CQ-4201673
...
* 在部署于JBoss® EAP 6.4上的AEM 6.3中，在触屏UI上创建页面时出现问题。NPR-17589：适用于CQ-4210137的修补程序
...
* 验证PDF文档是否符合PDF/A-1b规范时，AEM Forms返回验证错误：表单字段有多种外观。 PDF 文档符合 PDF/A-1b 标准。NPR-18013
...
* Microsoft® Excel和Microsoft® PowerPoint需要更长时间才能打开受AEM Document Security Extension for Microsoft® Office保护的文档。 CQ-4212358
...
AEM累积修补程序包6.3.0.1是一个重要更新，它包括自2017年4月AEM 6.3正式发布以来的若干内部修补程序和客户修补程序。 AEM 累积修补程序包的主要功能亮点包括：
...
## 通过Software Distribution下载CFP说明 {#download-instructions-for-cfp-via-package-share}
...
>[!NOTE]
>
>Adobe 提供的可选功能包依赖于发行版本和累积修订包。如果您已安装功能包，请联系 [AEM客户关怀团队](https://experienceleague.adobe.com/?support-solution=General#support) 以验证与AEM 6.3的累积修订包的兼容性。

>[!NOTE]
>
>我们建议您先对每个新的安装包运行验证，然后再尝试安装。预验证会分析并报告安装之前发现的任何错误，并主动向用户发出有关此类错误的警告。
>
>您可以在以下位置访问有关验证选项的文档 [https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)

* AEM 6.3.3.0 是安装 CFP 的先决条件。访问 [升级文档](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) 有关将AEM安装升级到AEM 6.3的详细说明。
* 对于使用RDBMK或MongoDB的群集部署，可以在使用包管理器的任何创作实例上安装CFP包。
...
* 您可以在默认位置添加日志记录器 [http://localhost:4502/system/console/slinglog](http://localhost:4502/system/console/slinglog) 具有以下属性：
...
### 通过Software Distribution安装累积修补程序包 {#install-the-cumulative-fix-pack-via-package-share}

1. 单击 [Software Distribution](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq640/cumulativefixpack/aem-6.4.8-cfp-2.0.zip) 链接，以便您能够下载包。
...
>[!NOTE]
>
>如果要安装6.3.3.0或更高版本，请执行以下过程来配置JBoss®应用程序服务器的设置。 如果您要在OracleWebLogic或IBM® WebSpehere应用程序服务器上运行的AEM Forms服务器上安装6.3.3.0，则无需进行其他配置。 有关更多详细信息，请参阅 [AEM 6.3.3.0 发行说明](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)。
...
* 安装AEM CFP 6.3.3.x期间可能会出现以下错误和警告，这些错误和警告可以安全忽略：
...
* 重新打开组件时，未在pathBrowser中选择PathField 。 NPR-24177
...
要手动更新adobeLiveCycle®cq -author.ear/ cq .war中的更改，请执行以下步骤：
...
* 从WebSphere®应用程序服务器中卸载之前的企业存档(EAR)文件，并按照的第10.2节中的步骤来安装更新后的EAR文件。 [https://helpx.adobe.com/pdf/aem-forms/6-3/install-single-server-websphere.pdf](https://helpx.adobe.com/cn/pdf/aem-forms/6-3/install-single-server-websphere.pdf)
...
`1)` 路径：“/content”
`a)` 主体：reference-adjustment-service类型：允许权限：jcr：read ， jcr：modifyProperties Restrictions ： rep：glob=&quot;/&#42;/jcr：content&quot;
`b)` 主体：reference-adjustment-service类型：允许权限：jcr：read ， jcr：modifyProperties Restrictions ： rep：glob=&quot;/&#42;/jcr：content/&#42;&quot;

`2)` 路径：“/content/usergenerated”
`a)` 主体：reference-adjustment-service类型：允许权限： `jcr:write`
...
默认值为 1。通过覆盖文件constants.js (`/libs/cq/ui/widgets/source/constants.js`)，取消对CQ.PAGE_PROPERTIES_MAX_RECURSION_LEVEL属性的注释，并为其指定所需值(页面的 `/jcr:content` ，其中存储了页面属性的数据)。

**如果用户必须创建多个页面变体，以使页面的 `/jcr:content` 节点数大于1000，请使用以下步骤进行配置更改：**

* 配置 Apache Sling 的属性 JSON MaxResults
* 使用 `/system/console/ configMgr` 获取 Servlet
* 将其值设置为大于> 1000（这是当前的默认值）的数字，以便该数字大于 `/jcr:content` 子树，直到达到上面配置的深度。
...
要在 Maven 项目中使用 Uber Jar，请参阅[如何使用 Uber Jar](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) 一文，并在项目 POM 中包含以下依赖项：
...
| 区域 | 专题 | 替换 | 版本号 |
|----|-----|-----|-----|
| Assets 和 Adobe Creative Cloud 集成 | AEM 6.2 中引入了 [AEM 到 Creative Cloud 的文件夹共享](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)功能，以便创意用户可以从 AEM 访问资产。Creative Cloud应用程序中发布的一项新功能AdobeAsset Link提供了更好的用户体验，以及更强大的直接从Photoshop、InDesign和Illustrator中访问AEM资源的功能。</br></br> Adobe没有对文件夹共享功能做出更多增强。 虽然此功能包含在AEM中，但建议使用替代功能。 | Adobe 资产链接或桌面应用程序。有关更多信息，请参阅 [AEM Creative Cloud 集成](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)一文。 | AEM 6.3.3.x |
