...
>[!NOTE]
>
>对于AEM Forms客户，请在安装任意AEM Service Pack、累积修补程序包或功能包之后，再安装AEM Forms附加组件包，这一点至关重要。
...
* 以PDF预览信件时，放置在主页面上的文本字段不遵循从数据选项卡或根据指定的数据链接输入的值。 NPR-29239：适用于 CQ-4266856 的修补程序。
...
* 增强了表单预填充服务的安全性
* 更新至最新的adobe-reader-extensions-dsc.jar文件，以获取reader扩展中的修补程序。
...
* 使用直接信件URL不会填充与XDP中定义的绑定对应的信件数据。 NPR-24145：适用于 CQ-4244290 的修补程序
...
* (OSGi) [JEE]使用“验证PDF进程”进行验证时，包含附件的PDF的签名状态不正确。 NPR-23269、NPR-23737
...
* (Digital Publishing Suite)作品集上传到DPS之前，“出版日期”和“封面日期”都是必填字段。 NPR-22484
...
* 转出已在主节点中移动的结构会导致错误的`cq:moveTarget`。 NPR-22373：适用于 CQ-4232536 的修补程序
...
* 在IE 11和Chrome中，自适应表单中的下拉列表占位符值不一致。 NPR-22405：适用于 CQ-4227096 的修补程序
...
* 选择组件不会在表单提交期间删除目标属性。 NPR-21163：适用于 GRANITE-14125 的修补程序
* 展开HTTP.encodePathOfURI会对加号“+”进行双重编码。 NPR-21417：适用于 GRANITE-16340 的修补程序

### 漏洞 {#vulnerability-4}

* DAM元数据编辑器中存在跨站点脚本(XSS)漏洞。 NPR-21434：适用于 CQ-83472 的修补程序
* 多个SWF文件容易遭受跨站点脚本(XSS)攻击。 NPR-20612：适用于 CQ-4213297 的修补程序
...
* 安装AEM 6.2 SP1-CFP9后，将禁用信件预览提交按钮。 NPR-21547
...
* docConvertor无法生成PDF/A，错误为“元素`stEvt:action`的前缀“stEvt”未绑定。” NPR-21032：适用于 CQ-4222540 的修补程序
...
* 如果在org.apache.sling.i18n 2.5.6中重新启动服务器，则国际化词典有效。NPR-20525：适用于Granite-19490的修补程序
* 没有根据使用默认线程转储收集器配置(默认的AEM启动)设置的计划程序时间段生成任何线程转储。 NPR-20288：适用于GRANITE-19488/GRANITE-12741 CQ-90647的修补程序。
...
* 增强了自适应表单功能，可将 PDF 作为附件附加到 Safari 中。此体验提供了一个新的Dynamic Media ，该新架构可连接到新的Dynamic Media发布基础架构，以实现更快、更灵活的复制。
...
* 当用户尝试发布被其他用户锁定的`/unpublish/create`版本页面时，AEM显示意外行为。 NPR-19249：适用于 CQ-4215298 和 CQ-4203856 的修补程序
...
* 将内容从Microsoft®Word或Web浏览器复制到通信管理器文本编辑器时，样式会丢失。 NPR-19530
* 文本编辑器中不含换行符的内容无法换行。 NPR-19481
...
* 即使用户没有复制权限，Publish操作按钮也可见。 NPR-17353：适用于 CQ-4209269 的修补程序
* 使用min：gcc；obfuscate=true启用缩小功能后，站点管理员和杂项管理员将无法正常运行。 NPR-18593：适用于 CQ-4209220 的修补程序
...
>
>* 从Word/Web复制粘贴到CM文本编辑器时，内容中显示换行符。 NPR-19530
...
* 无法使用远程Hardware Security Module签署PDF文档。 NPR-18017
...
* 使用经典 UI 和触控 UI 移动页面时，“移动”对话框显示的引用不超过 150 个，从而导致用户无法更新这些引用和重新发布页面。通过为经典UI引入可在站点管理员节点`'/libs/wcm/core/content/siteadmin'`上配置的“maxRefNo”属性，此问题已得到修复。 此属性指定在大量移动操作之前显示的最大引用数（默认值为 150）。如果某个页面有多个引用，则它们不会显示在 movePage 对话框中。此配置还适用于dam管理员和杂项管理员，方法是分别在以下节点上应用配置： `'/libs/wcm/core/content/damadmin'`和`'/libs/wcm/core/content/miscadmin'`。 NPR-17222：适用于 CQ-85878 的修补程序
...
* 通过在`/apps`中叠加来向&#x200B;**Assets OmniSearch**&#x200B;结果添加列或自定义列不起作用。 NPR-16737：适用于 CQ-4206785 的修补程序
...
* 使用Firefox或Google Chrome浏览器导航到&#x200B;**[!UICONTROL 全局导航]**&#x200B;窗格时，会获得不同的用户体验。 Firefox浏览器显示&#x200B;**[!UICONTROL 工具]**&#x200B;菜单。 Google Chrome浏览器显示&#x200B;**[!UICONTROL 导航]**&#x200B;菜单。 NPR-16770：适用于 CQ-4200456 的修补程序
...
* 在自适应Forms编辑器中，head.jsp中的“Target设置”注释应被替换为新的context hub语句。 NPR-17173
* 在自适应Forms规则编辑器中，**[!UICONTROL 选择项]**&#x200B;将事件显示为“null”。 NPR-17139
* 通过使用前进箭头(>)向前导航来重新提交已提交的表单。 NPR-17080
* 通过AJAX提交自适应表单时，如果发生错误，永远不会调用“error”回调函数。 NPR-17034
...
* 在&#x200B;**[!UICONTROL 默认预填充服务配置]**&#x200B;中支持`dataRef=C:/Users/`类型的正则表达式。 此类型用于预填充自适应Forms的数据。 NPR-16425
...
* 规则编辑器中的构造&#x200B;**[!UICONTROL 保存表单]**&#x200B;不起作用。 NPR-16905
...
* 对于已提交的实例，文件附件的`Upload Comments`不会显示在UI中，但它们存在于CRX存储库内所存储的XML中。 NPR-17075
...
* 输出服务无法处理被修改为在`date-time format`元数据中包含附加“milli-sec”字段的PDF。 NPR-16838
...
* 包管理器中的验证选项，用于检测`/apps`下叠加的文件(JSP或JavaScript文件)与`/libs`下的修补程序中包含的文件之间的任何冲突。 然后，受影响的叠加可以重新设置为包含来自`/libs`下文件的更改。 NPR-16216：适用于 CQ-81729 的修补程序
...
* 从 AEM 6.0 升级到 AEM 6.2 后，在搜索标记时，因大量查询而导致经典 UI 性能缓慢。要解决此问题，可以执行[在标记控制台中禁用复制状态（经典UI）](#disable-replication-status-in-tagging-console-classic-ui-npr)中所述的步骤。 NPR-15842：适用于 CQ-4201748 的修补程序.
...
* 在保存带有附件的自适应表单时，附件的完整路径（而不是附件的名称）会添加到生成的XML的`fileAttachment`标记中。 NPR-16529
...
* 导航到页面组件时，经典UI对话框中不会获取具有`tagBasePath`属性的标记的值。 NPR-15543：适用于 CQ-4199950 的修补程序
...
* 在“时间扭曲”模式下关闭“编辑”页面并尝试从站点管理员重新打开该页面时，会导致出现状态为`500`的错误。 NPR-14965：适用于 CQ-109647 的修补程序
* 在 Digital Asset Manager (DAM) UI 中，“用户选取器查找可授权项”搜索会导致“内存不足”异常。NPR-15307：适用于 CQ-98542 的修补程序

### 资源 {#assets-17}

* 在Omnisearch中搜索资产、选择资产并尝试通过单击&#x200B;**查看属性**&#x200B;来编辑属性后，**保存**&#x200B;按钮会将用户重定向到空白页面。 NPR-15900：适用于 CQ-4202372 的修补程序
...
* 在&#x200B;**[!UICONTROL 触屏UI]**&#x200B;中，当使用Internet Chrome浏览器版本56.0.2924.87时，对于现在具有type=&#39;datetime&#39;的日期字段，将显示保存的日期。NPR-15383：适用于GRANITE-16481的修补程序
...
* Omnisearch不允许使用`/apps`的客户添加列数据源，因为它假定位置配置列在&#x200B;*/libs/granite/omnisearch/content/metadata/*&#x200B;下。 NPR-13188：适用于 GRANITE-16479 的修补程序
...
* 使用“营销活动 — 定位”和`MAC` - Test and Target集成组件时，取消发布活动不会更新主UI中的活动状态。 NPR-15401：适用于 CQ-4199839 的修补程序
...
* 在基于XML架构的自适应表单中，提交的XML中存在`afData/afBoundData`包装器，即使预填充XML不包含`afData/afBoundData`包装器也是如此。 NPR-15541
...
* 从编辑器提交PageProperties对话框时，MSM Post处理器会写入请求中的一些参数，而不是`msm:writeLiveCopyConfig`参数。 NPR-14434
...
* 如果资产/文件夹URI过长，则选择多个资产会引发响应代码为414（请求URI过长）的错误。 NPR-13516：适用于 CQ-76076 的修补程序
...
* 自`Jquery`更新以来，用户无法选择资产收藏集并在内容片段的“关联内容”面板中确认选择。 NPR-14847：适用于 CQ-4194209 的补丁包
...
* 启动工作流时，用户无法从“工作流收件箱”控制台预览有效负载。 NPR-13226：适用于 CQ-100275 的修补程序
* 使用SAML身份验证处理程序配置的“saml_request_path”Cookie显示Cookie设置时带有额外的`?`字符。 此外，当 SAML 响应回发到 AEM 时，AEM“saml_request_path”Cookie 会因字符已编码而返回无效值。NPR-13517：适用于 GRANITE-11722 和 GRANITE-14414 的主动修补程序
...
* 在&#x200B;*FormsandDocuments*&#x200B;目录中导航时，当用户复制任何资产并导航到其他文件夹时，不会显示“粘贴”按钮。 CQ-111327
...
CFP12.1中包含了NPR-21355。如果要安装CFP12.1或更高版本，请执行以下过程以在JBoss®应用程序服务器上配置NPR-21355。 如果是在 Oracle WebLogic 或 IBM® WebSpehere 应用程序服务器上运行的 AEM Forms 服务器上安装 CFP12.1，则无需其他配置：
...
1. 在[AEM_Forms_Installation_directory]`/jboss/modules/system/layers/base/com/adobe/livecycle/main/`处创建`jsafeFIPS.jar`、`jsafeJCEFIPS.jar`和`certjFIPS.jar`文件的备份，并从先前提到的目录中删除这些文件。
...
* 由于重新启动存储库，发生诸如&#x200B;**创建组件实例**&#x200B;和&#x200B;**服务工厂返回null**&#x200B;等错误：
...
* 在包含智能标记功能包的AEM 6.2 SP1上安装CFPx时，将从DAM更新资产工作流中删除之前添加的智能标记Assets工作流步骤。

## Uber Jar {#uber-jar}

适用于6.2 SP1-CFP20的Uber Jar可在Adobe公共Maven存储库中获取。
...
>[!MORELIKETHIS]
>
>* [AEM 6.2 修补程序页面](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/aem-releases-updates)
>* [AEM 6.2 SP1 发行说明](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)
>* [AEM 6.2 发行说明](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)
>* [AEM 产品页面](https://business.adobe.com/products/experience-manager/adobe-experience-manager.html)
>* [AEM 6.2 文档](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)
>* [Adobe 优先产品更新](https://experienceleague.adobe.com/en/docs/release-notes/experience-cloud/current)
