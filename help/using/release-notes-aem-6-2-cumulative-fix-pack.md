...
* 在PDFG中将Jsafe Jar更新为CryptoJ 6.1.3.1。 NPR-21359
...
* 使用经典UI和触控UI移动页面时，“移动”对话框显示的引用不超过150个，从而导致用户无法更新这些引用和重新发布页面。 通过为经典 UI 引入“maxRefNo”属性，此问题已得到修复，该属性可在 siteadmin 节点“/libs/wcm/core/content/siteadmin”上配置。此属性指定在执行大量移动操作之前显示的最大引用数（默认值为150）。 如果页面具有多个引用，则它们不会显示在movePage对话框中。 此配置还适用于 damadmin 和 miscadmin，方法是分别在 `'/libs/wcm/core/content/damadmin'` 和 `'/libs/wcm/core/content/miscadmin'` 节点上应用配置。NPR-17222：适用于 CQ-85878 的修补程序
...
* 删除嵌套启动项中的启动项会导致子启动项被孤立。 NPR-17228：适用于 CQ-4202639 的修补程序
...
* 使用Google Chrome浏览器时，在从表单中删除附件后，尝试再次重新附加同一附件会引发错误。 NPR-17297
...
* API方法 `com.adobe.icc.services.api.LetterInstanceService` 无法进行导入。 NPR-17922、NPR-16008
...
* 使用 AEM Forms 6.2 输出服务将特定表单与数据 XML 合并所花费的时间，是使用 LiveCycle ES4 SP1 服务器执行相同操作所花费的时间的 20 倍。已在Windows和Linux®环境中修复该问题。 NPR-17501
...
* 此 **诊断工具** 从AEM 6.1 SP2就地升级到AEM 6.2 SP1后，页面不起作用。 NPR-17121：适用于 CQ-4196786 的修补程序
* HTL：在选择论坛、创建主题和帖子时， `Sightly SightlyCompiledScript` 添加的内容不正确 `addSelectors` 属性至 `RequestDispatcherOption`. NPR-17008：适用于 GRANITE-16384 的修补程序
...
* `Camera RAW` 文件(包括ARW、CR2、NEF、DNG和EPS)在资源中添加为演绎版后，无法选择或删除这些文件。 当用户单击这些文件时，会自动下载这些文件。NPR-16949：适用于 CQ-4206846 的修补程序
...
* 当从AEM创作环境预检发布时，超过5秒的预检操作会在AEMM - AEM PECS集成Splunk功能板上引起异常峰值，导致每秒出现大量状态请求。 NPR-16908：适用于 CQ-4207055 的修补程序
...
* 使用翻译创建的语言副本指向根主副本，而不是本地国家/地区Live Copy。 NPR-17257：适用于 CQ-4208287 的修补程序
...
* 通过AJAX提交自适应表单时，如果发生错误，则绝不会调用“error”回调函数。 NPR-17034
...
* 文档片段中开头或表达式之间有Tab空格的信件无法呈现或预览。 NPR-16769
...
* 单击 **[!UICONTROL 返回]** HTML工作区中某个表单的按钮不会关闭该表单，而是会将其返回到“组队列”。
NPR-16352
...
* 包管理器中的验证选项，用于检测/apps下叠加的文件（JSP或JavaScript文件）与/libs下的修补程序中包含的文件之间的任何冲突。 然后，受影响的叠加可以重新设置为包含对 /libs 下文件的更改。NPR-16216：适用于 CQ-81729 的修补程序
* 启动发布程序后，error.log中的日志记录有时会停止几秒钟，必须清除才能再次运行。 请求更新日志记录框架并提供 Sling 日志记录。NPR-15913：适用于 Granite-15452 的修补程序
...
* 使用定位时，发布营销活动不会自动发布整个树，包括品牌和主树。 NPR-15855：适用于 CQ-94630 的修补程序
...
* 对于具有文件附件小部件的自适应表单，预览附加文件后，表单的某些部分会被禁用。
NPR-16611

* 对于允许多个附件的文件附件小部件，如果在具有先前附件的小部件上提交了具有附件的新表单实例，则会显示错误代码。 打开添加的附件而非实际内容时出现此错误。 NPR-16258
...
* 使用Internet Explorer浏览器预览具有可滚动内容的信件时，浏览器的滚动条与信件的滚动条重叠。 因此，无法查看右侧片段的整个内容。 NPR-16068
* 使用 Google Chrome 浏览器创建或编辑文本文档片段时，颜色选择下拉列表会自动弹出，并且无法删除。用户必须选择列表作为数据条目类型才能编辑片段。 NPR-16067
* 使用时 `Letterinstance` API、方法 `import com.adobe.icc.services.api.LetterInstanceService` 不起作用。 NPR-16008
...
* 在AEM Forms Designer中，无法将词典拼写检查中的表单区域设置更改为“法语（加拿大）”。
NPR-15896
...
* 改进了翻译工作流；增强了Microsoft®连接器，可为Azure门户使用新的翻译API
* 修复了项目和营销活动中的问题
...
* 使用时，无法阻止潜在的空指针异常 `com.adobe.granite.infocollector.impl.FilesTraversal`函数调用。 NPR-15169：适用于 CQ-4197640 的修补程序
* 某些辅助节点的工作流状态不一致，并且在为该节点调度观察事件时显示错误。NPR-15701：适用于 GRANITE-13786 的修补程序
* 当用户在CRXDE中选择节点（例如/content/dam/），然后选择“访问控制”选项卡时（确保存在访问控制列表），拖放某些元素会移动元素，而不是选定的元素。 NPR-15696：适用于 GRANITE-16300 的修补程序
* 在尝试模拟时从下拉列表中选择用户，会导致整个用户弹出窗口消失。NPR-15774：适用于 CQ-4201738/GRANITE-11895 的修补程序
* 在 Omnisearch 中，无法按包含自动填充建议的标记进行搜索。NPR-15088：适用于GRANITE-14426的修补程序。
...
* 编辑 `TagInput` 字段组件不显示以前针对对话框字段配置的值。 NPR-15464：适用于 CQ-4200360 的修补程序
...
* 经典UI中的幻灯片放映组件无法创建幻灯片或拖放图像以创建幻灯片。 NPR-15281：适用于 CQ-4194164 的修补程序
* 无论权限如何，都会在站点Admin Console中向用户显示“创建”选项，例如“创建页面”、“创建站点”、“创建Live Copy”、“创建启动项”和“创建目录”菜单项。 NPR-15278：适用于 CQ-94436 的修补程序
* 安装 AEM 6.2 Service Pack 1 后，“包括子页面”滑块停止用于页面启动项。NPR-15230：适用于 CQ-4198449 的修补程序
* 请求增强版本清除功能，以便以块形式获取和处理版本，并能够在XPath查询中使用指定的路径。 NPR-15186：适用于 CQ-109205 的修补程序
...
* 在创建视频配置文件时，如果同时选择了高质量和中等质量的视频编码预设，则不会保存所做的更改。 NPR-15447：适用于 CQ-4195482 的修补程序
* 即使由于服务器错误响应导致将资源上传到Brand Portal失败，Brand Portal UI中的状态仍会更新为“已发布”，这样便难以跟踪缺失的文件。 NPR-15442：适用于 CQ-4197968 的修补程序
* 将资产文件夹发布到Brand Portal时，如果发布时间超过一小时，则某些文件无法发布。 NPR-15441：适用于 CQ-4199493 的修补程序
...
* 通过代理服务器进行连接时，将视频发布到YouTube。 NPR-15109：适用于 CQ-110332 的修补程序
* 在 data-sly-resource 中使用名称包含圆点或句点 (.) 的资产，不会解析到同一资产，输出路径将在圆点处终止。NPR-15069：适用于 CQ-4195914 的修补程序
* 将AEM 6.2升级到Service Pack 1后，将资源同步到Scene7失败。 此 `dam:Scene7FileStatus` 属性显示 `UploadFailed` 已发布资产的状态。 NPR-15269：适用于 CQ-4197708 的修补程序
...
* 使用翻译项目时，从英文主目录更新语言副本会生成11个单独的启动项，所有这些启动项的名称和源根均相同。 但是，如果页面名称遵循某种设置模式，则每个页面的启动根稍有不同。 NPR-15605：适用于 CQ-4200699 的修补程序
* 当语言根的名称中包含连字符和短划线时，无法为页面创建翻译项目。NPR-15171：适用于 CQ-96286 的修补程序
* 请求更新Microsoft®连接器，以便能够使用Microsoft®在Azure门户中提供的Microsoft® Translator API。 NPR-15320：适用于 CQ-101010 的修补程序
...
* 升级到最新版Java™ 1.8.0_121-b13后，在AEM Forms中无法访问管理员用户界面。 NPR-15330
...
* 恢复未保存的任务时，显示的关于恢复未保存任务的消息必须更加清晰，以减少用户错误。 NPR-15377
...
* WCM Sightly Foundation Parsys组件忽略组件声明标记名称， `cq:htmlTag / cq:tagName`. NPR-14225
* 在触屏UI中，当使用Sightly Parsys呈现通过JavaScript插入的组件时，自定义修饰在页面刷新后会被忽略。 NPR-14122
* 如果创建了多个富文本字段（例如链接），“目标”下拉列表在触屏UI对话框中无法使用。 NPR-13911
* 在触屏UI对话框中编辑具有多个富文本编辑器(RTE)属性的文本字段时，焦点会随机转移到特定RTE属性。 NPR-13703
...
* 从编辑器提交PageProperties对话框时，将重写LiveCopy选项卡中未更改的数据。 NPR-14693
...
* 无法正常显示具有 Repeat with Condition 表达式的资产。当用户预览 `*CDN3835RLCEN*` 书信模板，不显示正文目标区域中的资产。 当资产 `*VIPReassement*`，取消选择预先选定的可选资产，然后信件中会显示预先选定的其他资产。 NPR-14844
...
* 在基础元数据架构的下拉字段中为TypeHint使用布尔值时，批量编辑资源的元数据（追加模式）会生成错误。 NPR-14529：适用于 CQ-106876 的修补程序
...
* 当用户选择以下项中的所有选项时，资产报表页面将变得无响应： `Configure Columns` 对话框。 NPR-13187：适用于 CQ-95589 的修补程序
* Safari 和 Internet Explorer 中的标记选取器出现意外行为。NPR-13134
* 编辑从资产管理员搜索边栏中保存的搜索时，会将其另存为嵌套式智能选项，这将导致出现环境稳定性问题。 NPR-13119：适用于 CQ-99460 的修补程序
* 移动文件（或文件夹）并对其重命名后， `cq:name` 元数据不反映新文件名（文件夹名称）。 NPR-13036：适用于 CQ-99141 的修补程序
...
* 使用Coral-select组件(*granite/ui/components/coral/foundation/form/select*)，如果选定的值包含单个项目，则无法在Internet Explorer（IE 11或Edge浏览器）中正确初始化值。 NPR-13395：适用于 CQ-101013 的修补程序

### 项目 {#projects-5}

* 在导出使用翻译方法“human”和翻译提供程序“none”创建的翻译项目时，由于缺少GUID映射文件，因此不会生成translation_export_summary.xml文件。 NPR-13137：适用于 CQ-91976 的修补程序
* 在AEM项目中，如果创建的项目设置了到期日期属性，则日期转换设置的时间不正确，因为服务器和客户端之间存在时区差异。 NPR-13003：适用于 CQ-98288 的修补程序
...
* 改进了触屏UI中收件箱和通知徽章的性能。 NPR-14145：适用于 CQ-101125 的修补程序
...
* 无数 `AEM-Scene7` 在AEM激活期间创建和取消的sling作业在复制期间记录为存档作业。 NPR-12835：适用于 CQ-86115 的修补程序

### 安全 {#security-5}

* 请求解决 WCMDebug 筛选器中的输入验证问题。NPR-12444：适用于 CQ-94890 的修补程序请求
* 主动请求更正使用创建启动向导时的XSS行为。
...
* 搜索通信管理资产（如信件、文档片段或任何其他类型）时，工具栏中缺少“下载队列”图标。 NPR-14745
...
* 从用户界面批量导出通信管理资源时，AEM Forms Server会生成不必要的日志。 NPR-15226
* 预览信件时，两端对齐的文本以不同字体显示。NPR-15468

`**Forms Portal**`

* 从门户提交新的草稿后，Forms Portal中已提交表单的附件将不可见。 NPR-13515
...
* Adobe Acrobat Reader和Microsoft® Office无法打开受扩展身份验证保护的文档。 NPR-14482
...
* 没有为JBoss®+启用核心跨源资源共享(CORS)功能作为Servlet过滤器。 NPR-13050

## 通过Software Distribution下载CFP说明 {#download-instructions-for-cfp-via-package-share}

>[!NOTE]
>
>对于AEM Forms客户，请在安装任意AEM Service Pack、累积服务包或功能包之后，再安装AEM Forms附加组件包，这一点至关重要。

您可以直接从Software Distribution下载CFP包，或者执行以下步骤：
...
>[!NOTE]
>
>Adobe 提供的可选功能包依赖于发行版本和累积修订包。如果您已安装功能包，请联系 [AEM客户关怀团队](https://experienceleague.adobe.com/?support-solution=General#support) 以验证与AEM 6.2的累积修订包的兼容性。
...
* AEM 6.2 Service Pack 1 是安装 CFP 的先决条件。有关安装说明，请参阅 [AEM 6.2 Service Pack 1](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions).
...
### 通过Software Distribution安装CFP {#install-the-cfp-via-package-share}

要在现有AEM 6.2 SP1实例上安装累积修补程序包，请执行以下步骤：

1. 要下载包，请单击 [Software Distribution](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq640/cumulativefixpack/aem-6.4.8-cfp-2.0.zip).
...
1. “产品信息”页面(/system/console/productinfo)此时应在“已安装的产品”下显示更新的版本字符串“Adobe Experience Manager，版本6.2.0.SP1-CFP20”。
...
## DTM、Analytics、Target连接的用户可配置超时参数 {#user-configurable-timeout-parameters-for-dtm-analytics-target-search-promote-connections}
...
CFP12.1中包含了NPR-21355。如果要安装CFP12.1或更高版本，请执行以下过程以在JBoss®应用程序服务器上配置NPR-21355。 如果您是在OracleWebLogic或IBM® WebSpehere应用程序服务器上运行的AEM Forms服务器上安装CFP12.1，则无需进行其他配置：
...
1. 创建备份 `jsafeFIPS.jar`， `jsafeJCEFIPS.jar`、和 `certjFIPS.jar` 文件位于 [AEM_Forms_Installation_directory]/jboss/modules/system/layers/base/com/adobe/livecycle/main/并从前面提到的目录中删除这些文件。

联系人 [Adobe支持](https://experienceleague.adobe.com/?support-solution=General#support) 以便获取新的JAR文件。 将 [Adobe 支持团队](https://experienceleague.adobe.com/?support-solution=General#support)提供的 JAR 文件放在 [AEM_Forms_Installation_directory]/jboss/modules/system/layers/base/com/adobe/livecycle/main/

1. （仅限 Windows）修改 `[AEM_Forms_Installation_directory]/jboss/standalone.conf.bat` 或 `domain.conf.bat` 配置文件：

* 对于独立配置中的JBoss®服务器，请打开standalone.conf.bat进行编辑。
* 对于群集配置中的JBoss®服务器，请打开domain.conf.bat进行编辑。
...
* 对于独立配置中的JBoss®服务器，请打开standalone.conf进行编辑。
* 对于群集配置中的JBoss®服务器，请打开domain.conf进行编辑。
...
默认情况下，当用户声明任务时，不会为其他用户刷新共享队列的计数。 为此，Adobe引入了一个新属性。 请按照以下步骤在 AEM 实例上配置此属性：
...
* 在具有下列内容的图像的图像卡片上显示标题的行为变化 `dc:title` 属性设置为字符串 [] （多字段）：尽管所有标题都保存在CRX中，但UI中的图像卡片上只显示最新更改的标题。 适用于 CQ-4217165 的修补程序
...
* 在Mongo和DB2®中的CFP安装中发现错误： **org.apache.sling.discovery.oak.TopologyWebConsolePlugin addDiscoveryLiteHistoryEntry：异常： java.lang.NullPointerException**. 在 CFP8 上安装 CFP 后，不会出现此错误。
* (AdobeGranite维护计划程序更新任务)com.adobe.granite.maintenance.impl.TaskScheduler：未找到名为WorkflowPurgeTask for window的维护任务 `granite:weekly`
...
* 在包含智能标记功能包的AEM 6.2 SP1上安装CFPx时，将从DAM更新资产工作流中删除之前添加的智能标记资产工作流步骤。
