---
title: AEM 6.2 累积修订包
description: 了解有关 AEM 6.2 累积修复包发行说明的更多信息。
...
>[!NOTE]
>
>* AEM 累积修订包 6.2 SP1-CFP10 中包含一个新的 Sling `discovery- api` 包 Johnzon 1.0.0。此外，还为 CRX 存储库中的节点 */var/discovery* 添加了拥有读取和写入权限的服务用户 sling-discovery。
>
...
>
>* 对于在 AEM 实例上拥有许多用户的客户，Adobe 建议通过安装文件夹来部署 CFP。
>
...
* 在 6.2.1 CFP 17 及更高版本中，Image Preset 编辑器工具无响应。NPR-28147：适用于 CQ-4261041 的修补程序
...
* &lt;br>在内容片段中复制粘贴文本时，插入多余的。NPR-26660：适用于 CRTE-151 的修补程序
* 经典 site admin 不会在某些页面的右侧窗格中呈现列表。NPR-27247：适用于 CQ-4251621 的修补程序
...
* 从资源 UI 启动工作流时，用户需要具有写入/删除/修改权限。NPR-25688：适用于 CQ-4250140 的修补程序
* 即使用户没有“复制”权限，他们仍可看到“发布”和“取消发布”按钮。NPR-25094
* （工作流）智能标记资源工作流不会通过 AEM 代理配置进行处理。NPR-25915：适用于 CQ-4248202 的修补程序
...
* RTE 富文本编辑器工具栏包含选择列表。NPR-25165：适用于 CQ-4248948 的修补程序
...
* 使用 OOTB Target 集成，定位组件时会呈现整个页面，而不是空的目标组件。NPR-25273：适用于 CQ-4248003 的修补程序
...
* 使用选择退出功能时呈现空白。NPR-25295：适用于 CQ-4246792 的修补程序
...
* cq -social-as-provider 软件包不从 Mongo 驱动程序 3.x 版本启动。NPR-26271：适用于 CQ-4252710 的修补程序
...
* 发布页面时，不相关的表单资源报告为引用。NPR-26167：适用于 CQ-4251004 的修补程序

### Forms - JEE 安装程序 {#forms-jee-installer-3}

#### 文档安全 {#document-security}

* 变量以数据类型“列表”填充，子类型为字符串，但会出现“无法强制对象”错误。NPR-26194：适用于 CQ-4252287 的修补程序
...
* (Firefox/Chrome) 无法在“资源共享”页面中下载资源。NPR-24523：适用于 CQ-4224408 的修补程序
...
* 当一个请求包含无效查询参数时，LineChecker 无法将链接外部化。NPR-24127：适用于 CQ-4241893 的修补程序
...
* 在项目上启动“请求启动”工作流时，启动创建失败。NPR-24255：适用于 CQ-4245030 的修补程序
...
* 由于管理员搜索功能中的请求过多，观察到 CPU 使用率很高。NPR-23588：适用于 Granite-21286 的修补程序
...
* 通过默认预览来预览信件时，布局片段映射不会显示，但单击“预览”时布局片段映射会正确显示。NPR-22993：适用于 CQ-4237745 的修补程序
...
* Foundation 表中的主动安全修复旨在保持设计的一致性。
* 添加了对 TypeHint 的支持，可将值保存为字符串。
* 增强了表单预填充服务的安全性
* 更新至最新的 adobe-reader-extensions-dsc.jar 文件，以获取 Reader 扩展中的修复程序。
...
* (Dynamic Media) 选中“动态演绎版”复选框时，下载的 zip 文件会生成带有零字节文件的原始 TIFF 图像。NPR-22410：适用于 CQ-4198471 的修补程序
...
* 在转出具有多个转出配置的 Live Copy 时，如果根目录下出现 ResourceNameConflict，则转出流程将会终止，而不包括所有分支。NPR-22842：适用于 CQ-4236188 的修补程序
...
* 在翻译配置上设置“不翻译”资源时，将会创建资源语言副本。NPR-22540：适用于 CQ-4237962 的修补程序

### 用户界面 {#user-interface-1}

* 将 Omnisearch 与连字符查询一起使用，会返回服务器错误。NPR-22999：适用于 Granite-19674 的修补程序
* DatePicker 不支持由隐藏字段设置的手动设置外部类型提示。更改类型提示会导致转换错误。NPR-23333：适用于 Granite-21194 的修补程序

### WCM - Foundation 组件 {#wcm-foundation-components-2}

* 为了提高安全性，CAPTCHA 组件现已弃用。如果您使用 CAPTCHA 组件，安装 6.2 SP2-CFP15 或更高版本后会显示消息“Captcha 组件已弃用，不应使用”。可以自定义 AEM 组件来包含 reCAPTCHA 以提高安全性。NPR-22151：适用于 CQ-4220052 的修补程序
...
* 通过默认预览来预览信件时，布局片段映射不会显示，但在单击“预览”按钮时，布局片段映射会正确显示。NPR-23335：适用于 CQ-4237745 的修补程序
...
* 重新发布 Site 页面时，不会发布嵌入到 Site 的 Forms。NPR-23014：适用于 CQ-4236566 的修补程序

#### 漏洞 {#vulnerability-3}

* 跨站点脚本的主动修补程序。NPR-20624：适用于 CQ-4206055 的修补程序
* Forms Manager 的注释选项卡中存储的跨站点脚本 (XSS) 漏洞。NPR-27157：适用于 CQ-4255556 的修补程序

#### 加密服务 {#encryption-service}

* (OSGi) [JEE] 使用“验证 PDF 流程”进行验证时，包含附件的 PDF 的签名状态不正确。NPR-23269、NPR-23737
...
* 用户无法下载带有免责声明和长文件名的资源。NPR-22163：适用于 CQ-4235274 的修补程序
* 单引号字符会阻止批量视图中元数据的更新，当您使用快速工具栏操作打开资源属性时，用户界面会损坏。NPR-22317、NPR-22353：适用于 CQ-4236990、CQ-4236469 的修补程序
...
* （Touch UI）在命名空间下无法显示超过 40 个标记。NPR-22290：适用于 CQ-99114 的修补程序
* 从端口 -1 到 AEM 6.2 的 RSS 馈送问题。NPR-22158：适用于 CQ-4233339 的修补程序
* （IE）首次在富文本字段中创作任何字符时，将向该字符添加尾随空格。NPR-22443：适用于 CQ-4235343 的修补程序
* 当尝试匹配包名称时，由于包声明中存在尾随空格字符，Java™ Use 对象冻结 SightlyJavaCompilerService。NPR-22557：适用于 Granite-20836 的修补程序
* 触屏 UI 控制台不会选取新的标记语言。NPR-22250：适用于 CQ-4239194 的修补程序

### Mobile On-Demand {#mobile-on-demand}

* (Digital Publishing Suite) 在作品集上传到 DPS 之前，出版日期和封面日期都是必需的字段。NPR-22484
...
* Target 引擎（mbox.js、at.js）不使用损坏的 URL而是使用包含冒号的 URL，这在某些部署中可能会失败。NPR-22366：适用于 CQ-4237854 的修补程序
...
* 工作流启动器无法接受字符串的 TypeHint 值。NPR-22436：适用于 CQ-4237855 的修补程序
...
* 在 IE11 与 Chrome 中，自适应表单的下拉列表占位符值不一致。NPR-22405：适用于 CQ-4227096 的修补程序
...
* （HTML 工作区）当用户领取任务时，仅针对该特定用户刷新队列计数。也就是说，除非刷新页面。NPR-19778：适用于 CQ-4233665 的修补程序
...
* 当 AT.js 用作客户端库时，在“目标组件设置”中启用“静态参数”字段配置。
* 修复了下拉组件的显示/隐藏功能。
...
* 版本清除无法删除旧版本的资源。NPR-21682：适用于 CQ-4212996 的修补程序
...
* (Safari) 用户无法在 Asset Finder 面板中查看所有资源。NPR-21981：适用于 CQ-4213720 的修补程序
* 时间扭曲返回一个带有乱码页面的“RecursionTooDeepException”错误，即使更改了日期也不会创建任何新版本。NPR-21707：适用于 CQ-4199536 的修补程序
* 在编辑器中加载页面时，WorkflowStatusprovider (pageinfo.json) 会被加载三次，从而导致 AEM 实例执行缓慢。NPR-21778：适用于 CQ-59232 的修补程序
...
* 用户在 mbox 调用中看不到测试静态参数值，而在使用 AT.js 作为云配置中客户端库进行测试时，则可以看到这些值。NPR-21930：适用于 CQ-4234520 的修补程序

### 平台 {#platform-4}

* 当用户或组的数量较大时，用户同步的性能出现问题。NPR-20431：适用于 CQ-4223282 的修补程序
* 用户不能使用 Sling Distribution 与用户同步。NPR-21911：适用于 Granite-20404 的修补程序
* 阻止在搜索摘录中高亮显示停止词（在 Geometrixx 页面上）。NPR-21835：Granite-21067 的热修复
注意：此修复需要 Oak CFP 1.4.20 或更高版本。

### 翻译 {#translation-3}

* 在创建翻译项目时，启动程序用户 ID 缺少 jcr 属性。NPR-21715：适用于 CQ-4230713 的修补程序
...
* 改进了包含多个嵌套组件的页面的呈现。
* 改进了 AEM 与 Adobe Digital Publishing Suite 之间资源出版日期的同步。

### 资源 {#assets-7}

* OmniSearch 中的字符过多导致 AEM 服务器崩溃。NPR-21083：适用于 CQ-4223602 的修补程序
* 在元数据架构的多值字段中的第二个选项中指定的值不会附加到 CRX-de 中先前指定的值。NPR-21220：适用于 CQ-4224526 的修补程序
...
* 添加 Target 框架时，在编辑器的“选择模式”列表中无法使用“目标选择”模式。NPR-21047
...
* 升级到 AEM 6.3 后，无法找到引用作为脚本实施的标记的自定义标记库。NPR-20149：适用于 Granite-18433 的修补程序

### 翻译 {#translation-4}

* 翻译工作流失败，因为 lang_country 代码超过 2 个字符。NPR-21088：适用于 CQ-4197439 的修补程序
* 在项目完成之前，请勿再次向翻译项目提交资源页面。NPR-21219：适用于 CQ-4209908 的修补程序

### 用户界面 {#user-interface-3}

* 选择组件不会在表单提交过程中删除目标属性。NPR-21163：适用于 GRANITE-14125 的修补程序
* HTTP.encodePathOfURI 扩展会对加号“+”进行双重编码。NPR-21417：适用于 GRANITE-16340 的修补程序

### 漏洞 {#vulnerability-4}

* DAM 元数据编辑器中的跨站点脚本（XSS）。NPR-21434：适用于 CQ-83472 的修补程序
* 多个 SWF 文件容易受到跨站点脚本 (XSS) 的攻击。NPR-20612：适用于 CQ-4213297 的修补程序
...
* 安装 AEM 6.2 SP1-CFP9 后，信件预览提交按钮被禁用。NPR-21547
* 选择资源链接类型时，“资源选择器”窗口不会在“编辑信件数据绑定”向导中打开。NPR-21164：适用于 CQ-4194567 的修补程序
...
* 更新了 cq-msm-core，以实现有效的 Live Copyindex 同步。
* 提高了内容片段的编辑效率。
* 它在包管理器中提供一个用于检测 ACL 权限的验证选项。
* 引入了营销活动功能，可为客户通信包含电子邮件 ID。
* 增强了 Dynamic Media 文件的视频编码功能。
* 修复 Sightly 组件和 LiveCopy 中的问题。
...
* 用户无法下载带有免责声明和长文件名的资源。NPR-20255：适用于 CQ-4222139 的修补程序
...
* 在 RTE 中使用链接插件关联一个已编码的资源文件时，会返回空链接。NPR-20626、NPR-21059：适用于 CQ-4223011 的修补程序
...
* 修复了 AEM 中 OOTB 资源的 curl Head 请求问题。NPR-20511：适用于 CQ-4221520 和 CQ-103024 的修补程序
...
* （Correspondence UI）单击一下后会禁用提交按钮。NPR-21078

### Forms JEE 安装程序 {#forms-jee-installer-9}

#### 汇编程序服务 {#assembler-service-1}

* docConvertor 无法产生 PDF/A，出现“未绑定 `stEvt:action` 元素的前缀‘stEvt’”错误。NPR-21032：适用于 CQ-4222540 的修补程序
* 调用服务 OMPFSubmission/PDFA/PDFtoPDFA 时出现名为 `java.lang.IllegalArgumentException message:No enum constant com.adobe.internal.pdfm.docbuilder.signature.PathValidationFailureReason.SIGNED_IN_FUTURE` 的异常。此异常会阻止短暂的签名验证过程在重新启动服务器之前无法完成。NPR-20792
...
* 添加了一个实用程序函数 onDialogLoaded 用于测试。
...
* 标题中包含特殊字符的缩略图资源在 NPR-20347 alt 属性中显示 Java™ 对象：CQ-4223620 的修补程序
...
* 如果在 org.apache.sling.i18n 2.5.6 中重新启动服务器，国际化词典将起作用。NPR-20525：Granite-19490 的修补程序
* 使用默认线程转储收集器配置（默认的 AEM 启动）时，不会根据调度程序周期生成任何线程转储。NPR-20288：适用于 GRANITE-19488/GRANITE-12741/CQ-90647 的修补程序。

### Sites {#sites-9}

* 如果在打开已保存的搜索后更改修改日期过滤器，则不会对结果产生影响。并且，显示的结果与修改日期过滤器的之前保存的值相同。NPR-19739：适用于 CQ-4219425 的修补程序
...
* （Touch UI）通过经典 UI 控制台删除启动项会使所有页面无法编辑。NPR-20520：适用于 CQ-4225074 的修补程序
* “目标”下拉列表不适用于对话框中的多个 RTE 组件。NPR-20345：适用于 CQ-4220981 的修补程序

### 平台 {#platform-7}

* 使用匿名会话进行访问时，ClientLibraryProxyServlet 不会将请求代理到发布实例上的客户端库，引发“HTTP 404 未找到”错误。NPR-20195：适用于 Granite-14409 的修补程序
...
* 在同一页面创建产品时，不会出现确认或重定向弹出消息。NPR-20257：适用于 CQ-4223414 的修补程序
...
* 即使从多个图像就地编辑器组件中删除图像后，ImageMap 属性仍然存在。NPR-20142：适用于 CQ-4222982 的修补程序
...
* 当通过 UI 更改 DropDownList 时，valueCommit 脚本会执行两次。NPR-19989：适用于 CQ-110212 的修补程序
...
* 当移动设备表单配置为以 PDF 格式查看时，预填充的表单不起作用。NPR-20566
...
* 修复了 Context Hub 的独立持久性缓存。
* 准确计算资源维度。
* 优化了将资源发布到 Brand Portal 时的 AEM 性能。
* 修复了画布节点中的 `Resourcetype` 值。
* 为文档片段内容启用了区分大小写和特殊字符搜索功能。
* 增强了自适应表单功能，可将 PDF 作为附件附加到 Safari 中。这种体验提供了一种新的 Dynamic Media，可连接到新的 Dynamic Media Publishing Infrastructure，以实现更快、更具可扩展性的复制。

### 资源 {#assets-10}

* AEM Assets 无法提取 InDesign 资源的子资源引用，包括指向资源的重复链接。NPR-19006：适用于 CQ-4204186 的修补程序
* 排序选项不适用于商务下的收藏集内的资源。NPR-19508：适用于 CQ-4213622 的修补程序
* 当某个与预先存在的资源具有相同名称的资源被移动到同一位置时，资源的 `cq:lastReplicationAction` 值会在二者之间交换，这会导致创建错误的元数据。NPR-19531
* 发布大量资源时，尽管所有资源均正确发布，仍会显示一条错误消息。NPR-19629：适用于 CQ-4219611 的修补程序
* 静态演绎版以固定尺寸列出，并不反映演绎版的真实大小。NPR-20004
* 将多个资源（超过 4 个）发布到 Brand Portal 时，AEM 实例运行缓慢。NPR-20009

### 站点 {#sites-10}

* 当用户尝试发布被另一个用户锁定的页面`/unpublish/create`版本时，AEM 会出现意外行为。NPR-19249：适用于 CQ-4215298 和 CQ-4203856 的修补程序
* 在手动提升嵌套启动时，子页面会被删除。NPR-19704
* 当 ContextHub 存储在初始化期间覆盖默认持久层时，会出现持久性问题。NPR-19979：适用于 CQ-4218399 的修补程序
* 内容片段与其他按钮重叠。NPR-19980：适用于 CQ-4221519 的修补程序
* 在 SiteAdmin 中使用别名查看站点页面时，不会显示该页面。NPR-20053：适用于 CQ-4221478 的修补程序
...
* 上传同一文件夹中的多个资源时，Internet 浏览器 11 和 Edge 出现渲染问题。NPR-20062
...
* 启用通过 tagadmin/tagging 控制台将标记从 AEM 发布到 Brand Portal 功能。NPR-20271
...
* 如果自适应表单的某个字段上的 validationState 未定义，并且发生 elementFocusChanged 事件，则会向 Adobe Analytics 服务器返回错误事件 (errorState)。NPR-19513
...
* 在 Safari 浏览器的智能收藏集中应用 Omnisearch 筛选器时出现问题。NPR-19511
* 当有多个与 PDF 资源关联的关键字时，PDF 关键字元数据无法正确提取，并且会以错误方式修改。为解决该问题，已删除 PDF 资源的“主题”字段元数据属性。但是，您可以编辑元数据架构，为“主题”字段添加多值文本字段。NPR-19126
* 工作流通知服务不对电子邮件中的链接进行编码，从而导致用户在单击链接后，无法加载链接。NPR-19490：适用于 CQ-4218055 的修补程序
* 无法使用 Chrome 在列视图中加载完整的页面/资源列表。NPR-19458：适用于 CQ-4214248 的修补程序
* 激活“请求激活”工作流时，AEM 收件箱中会显示不正确的关闭时间图标。NPR-19365：CQ-4216174
* 在列表视图中排序时出现问题。NPR-19217：CQ-95602
* 在“资源文件夹”设置中更改标题或缩略图时，文件夹的原始组和权限将被覆盖。NPR-19283：适用于 CQ-4216080 的修补程序
...
* 工作流实例卡在“Salesforce.com 导出”工作流步骤上。NPR-19222：适用于 CQ-4212976 的修补程序
* 无法编辑 Live Copy 创建的语言副本页面。NPR-18967
...
* 如果对目标选择模块进行编辑并保存超过一次，则目标选择模式下不显示定位对话框。NPR-19144：适用于 CQ-4216708 的修补程序
...
* 修复了 Dynamic Media Cloud Services、Touch UI 和Security UI 界面的性能问题。
* 修复了 Apache Felix Http Bridge 3.0.8 中的问题。
* 解决了创作与发布环境之间的无二进制复制 (BLR) 问题。
* 支持 Target Library 文件 AT.JS，这是一个用于与 Adobe Target 进行客户端集成的实现库，专为典型的 Web 实施和单页应用程序而设计
...
* 重新启动 AEM 实例后，页面上资源的“虚 URL”设置不起作用。NPR-18634：适用于 Granite-18085 的修补程序
* 在触屏 UI 中，会向没有资源发布权限的用户显示“发布”按钮。NPR-18620：适用于 CQ-4214042 的修补程序
* 为资源设置许可协议后，该资源的下载对话框中即不再有动态演绎版选项。NPR-18607：适用于 CQ-4212342 的修补程序
* 无法为名称中包含空格的资源下载动态演绎版。NPR-18571：适用于 CQ-4211738 的修补程序
* 与 Creative Cloud 共享资源文件夹时，无法保存多个用户。NPR-18489：适用于 CQ-103297 的修补程序
...
* 使用 min:gcc;obfuscate=true 启用缩小功能后，Site admin 和 Misc admin 无法运行。NPR-18593：适用于 CQ-4209220 的修补程序
* 每次刷新屏幕之前，不会显示自定义菜单项。NPR-18500：适用于 CQ-4213581 的修补程序
* 将 moment.js 升级至 2.10.6。NPR-18596：适用于 Granite-11881 的修补程序
* 应用 DM 宏的权限会破坏管理员用户的视图。NPR-18544：适用于 CQ-4211729 的修补程序
* 稍后发布资源会引发非法参数异常。CQ-4214532

### 站点 {#sites-12}

* 在具有 MongoDB 的双活作者群集上，当时间达到为内容设置的“按时”时，两位作者都会尝试触发对同一内容的复制。NPR-18708：适用于 CQ-4210982 的修补程序
...
* CR 安装程序在包安装后不会更新软件包版本。NPR-18728：适用于 NPR-15135 的修补程序
* 在创作与发布环境之间对二进制文件执行无二进制复制 (BLR) 失败。NPR-18704
* AEM 环境中的 Apache Felix Http Bridge 解析请求。NPR-18297
...
* 图像映射不会保留 HTL 图像组件中的选定坐标。NPR-18530：适用于 CQ-4211584 的修补程序
...
* 启用通过 tagadmin/tagging 控制台将标记从 AEM 发布到 Brand Portal 功能。CQ-4212165
...
>
>* 从 Word/Web 复制并粘贴内容到 CM 文本编辑器时会显示换行内容。NPR-19530
>* CM 文本编辑器中没有换行符的内容无法换行。NPR-19449
>

#### 自适应表单 {#adaptive-forms-9}

* 在为可重复面板添加新面板时，前一个面板中的下拉字段的值将被删除。NPR-18772
* 标记为仅接受整数的自适应表单字段，也接受从数字键盘输入的几个特殊字符。NPR-18680
* 用于在 guid eroot 面板的初始化事件中更改按钮标题的脚本不起作用。NPR-18476
* 在规则编辑器下创建的规则的右侧面板中看不到滚动条。NPR-18716
...
* PDF Generator 无法生成具有指定书签级别的 PDF 文档。适用于 CQ-4211102 的修补程序
...
* “时间线”控制台无法提供检出操作/工作流程，在“资源”文件夹中添加了大约 80 个片段后。NPR-18257：适用于 CQ-4211214 的修补程序以及 NPR-18251：适用于 CQ-4211216 的修补程序。
...
* 为 IP 范围设置分段规则式，个性化仅对第一个 IP 有效。NPR-18121：适用于 CQ-83767 的修补程序
* 启用 historyShow 属性后，登录由于 NumberFormatException 而失败。NPR-18073：适用于 CQ-101965 的修补程序
*  中显示标记的已删除页面。标记的已删除页面在 Touch UI 中可见。NPR-18025：适用于 CQ-86694 的修补程序
* 加载包含大量（2000 及以上）受众的页面时，出现性能问题。NPR-17884：适用于 CQ-4209567 的修补程序
* 删除页面上的另一幅图像后，无法选择该图像。NPR-17711：适用于 CQ-4201323 的修补程序

### 平台 {#platform-11}

* 对于没有所需权限的用户，Touch UI 控件不会隐藏。NPR-17945：适用于 CQ-4211231 的修补程序
* 标记选取器字段中缺少日文标记。NPR-17768：适用于 CQ-4210456 的修补程序
* 启用 FastQuerySize 后，getsize() 查询返回错误结果。NPR-18018
* 备用实例上的 Web Console 不可访问。NPR-17861：适用于 Granite-14582 的修补程序

### 商务 {#commerce-2}

* 当目录 Blueprint 没有为某个部分定义任何条件时，会发生查询遍历。NPR-18229：适用于 CQ-4211924 的修补程序
...
* 缺少复选框，因此您可以恢复继承。NPR-17753：适用于 CQ-4210139 的修补程序请求
...
* 请求使用 Apache Sling API 中 org.apache.sling.servlets.post servlet (2.3.22) 的升级版本来预先制止 XSS 漏洞。NPR-18963
...
* 当 HTL 图像组件呈现时，URL 中的文件名将重命名，从而导致 URL 损坏。NPR-17908：适用于 CQ-4211587 的修补程序
...
* 在创建信件时，信件说明未保存。NPR-18039
* 当文本模块已保存且文本模块中的表达式不包含开放或闭合的表达式标记时，未显示错误消息。文本模块显示错误消息，并且无法在信件中呈现。NPR-17798
* 安装附加包时日志中出现意外错误。NPR-18295

**Forms Manager**

* AEM Forms UI 按照时间以升序（最早的排在最前面）列出所有资源。用户无法按照时间以降序（最新的排在最前面）重新排列资源。NPR-18451
...
* 改进了操作栏在显示工具栏操作和 Coral 组件时的响应性。

### 资源 {#assets-14}

* 改进了“发布至 Brand Portal”功能的性能。NPR-17189：适用于 CQ-4204150 的修补程序
* 使用“共享链接”选项共享资源时，不会创建具有平面文件夹结构的 zip 文件以供下载。NPR-17513：适用于 CQ-4209381 的修补程序
* 选择 DAM 中的资源并单击“发布”时，“资源详细信息”页面中不显示“发布至 Brand Portal”选项。NPR-17351：适用于 CQ-94905 的修补程序
* 在 DAM 工作流步骤中，从 Session 或 ResourceResolver 获取的二进制流必须在最后的块中关闭。这样做可确保不会发生任何资源泄漏。NPR-17385：适用于 CQ-4209452 的修补程序
* 在 DAM 中上传 Word 文档会引发空指针异常，并且工作流实例停留在“正在运行”状态。NPR-17160：适用于 CQ-4207358 的修补程序
* 非管理员用户可以在元数据编辑器页面上看到已过期资源的“共享”、“移动”、“发布”和“下载”按钮。NPR-16903：适用于 CQ-101440/CQ-104535 的修补程序
* 在“资源”控制台中，应向管理用户显示“共享”、“移动”、“发布”和“复制”等操作。NPR-16902：适用于 CQ-4207111 的修补程序

### 站点 {#sites-14}

* 使用经典 UI 和触控 UI 移动页面时，“移动”对话框显示的引用不超过 150 个，从而导致用户无法更新这些引用和重新发布页面。该问题已得到解决，方法是引入一个可在站点管理节点 `'/libs/wcm/core/content/siteadmin'`上配置的经典 UI 属性“maxRefNo”。此属性指定在大量移动操作之前显示的最大引用数（默认值为 150）。如果某个页面有多个引用，则它们不会显示在 movePage 对话框中。此配置还适用于 dam admin 和 misc admin，方法是分别在 `'/libs/wcm/core/content/damadmin'` 和 `'/libs/wcm/core/content/miscadmin'` 节点上应用配置。NPR-17222：适用于 CQ-85878 的修补程序
...
* 在慢速连接中，选择资源并单击“开发人员工具”并不总是会在操作栏中显示工具栏操作，并且必须重新加载页面。NPR-17568：适用于 CQ-108365 的修补程序
...
* 如果为 AB 测试配置了自适应表单，单击“开始 AB 测试”不会开始测试，并且会引发浏览器控制台错误。NPR-17838
...
* 使用 AEM forms 6.2 输出服务将特定表单与数据 XML 合并需要花费 20 倍的时间。现在已在 Windows 和 Linux® 环境中修复该问题。NPR-17501
...
* 无论使用 JJEE Admin UI 控制台还是 OSGi 控制台，表单用户的功能都应相同。NPR-17670
...
* 预览信件期间在 Text Editor 中编辑片段时，应显示已处理的文本（而不是片段中使用的内嵌条件）以进行编辑。NPR-15748、NPR-17504
...
AEM 累积修订包 6.2 SP1-CFP4 是一个重要更新，它包括自 AEM 6.2 SP1 正式发布以来的一些关键客户修补程序。

此累积修订包的主要功能亮点包括：

* 修复了 Camera Raw 文件演绎版的资源、时间线视图和图像方向
* 改进了
...
* 通过叠加来向 **Assets OmniSearch** 结果添加或自定义列 `/apps` 不起作用。NPR-16737：适用于 CQ-4206785 的修补程序
...
* 用户管理屏幕上显示的结果数量在卡片和列表视图中不同。NPR-16241：适用于 GRANITE-16914 的修补程序
...
* 在 Assets UI 中在另一个 PDF 内创建一个 PDF 不会在 DAM UI 中显示创建的 PDF。但是，PDF 在 CRX 存储库中可见。NPR-16833：适用于 CQ-4206501 的修补程序
...
* WCM 启动项提升需要较长的时间才能提升包含大型页面树的启动项。NPR-16480：适用于 CQ-82731 的修补程序
* 当使用非有效或未完成的规则创建区段（或受众）时，`ClientContext` 区段条件呈现器崩溃。NPR-16759：适用于 CQ-4205104 的修补程序
* 在 WCM 启动项中，当从Touch UI 的页面属性中启动操作时，与启动项关联的页面不会取消发布。NPR-16560：适用于 CQ-4204681 的修补程序
...
* 使用 Firefox、Safari或 Google Chrome 浏览器导航到&#x200B;**[!UICONTROL 全局导航]**&#x200B;窗格时，会获得不同的用户体验。Firefox 浏览器显示 **[!UICONTROL 工具]** 菜单。Google Chrome 浏览器显示 **[!UICONTROL 导航]** 菜单。NPR-16770：适用于 CQ-4200456 的修补程序

### 营销活动 {#campaign}

* 测试 AEM 营销活动模板和修改种子地址以包含“其他数据”时，Adobe Campaign 下拉列表会在Touch UI ContextHub 中消失。NPR-16771：适用于 CQ-105748 的修补程序

### Mobile on-demand {#mobile-on-demand-3}

* 当从 AEM Author 预检发布时，超过 5 秒的预检操作会在 AEMM - AEM PECS 集成 Splunk 功能板上引起异常峰值，导致每秒出现大量状态请求。NPR-16908：适用于 CQ-4207055 的修补程序
...
* 使用“翻译”创建的语言副本指向“根主版”，而不是本地国家/地区 Live Copy。NPR-17257：适用于 CQ-4208287 的修补程序
...
* 在自适应表单编辑器中，head.jsp 中的 Target 设置注释应替换为新的 Context Hub 语句。NPR-17173
...
* 支持 `dataRef=C:/Users/`默认预填服务配置 **[!UICONTROL 中]**&#x200B;类型的正则表达式。此类型用于自适应表单数据的预填。NPR-16425

* 如果存在嵌套延迟加载情况，则无法正确触发所有面板的验证。NPR-15821

**通信管理**

* 如果信件包含空文本模块（无任何文本的模块），则信件演绎版失败。NPR-17054
...
* `Upload Comments` 文件附件不会显示在已提交实例的 UI 中，尽管它们存在于存储在 CRX 存储库中的 XML 中。NPR-17075
...
* 单击 HTML Workspace 中某个表单的&#x200B;**[!UICONTROL “返回”]**&#x200B;按钮，不会关闭该表单，而是会将其返回到“组队列”。NPR-16352
...
* 修复了在Touch UI 中使用定位同步受众、发布营销活动，以及选择目标量度的问题
* 解决了翻译作业的同步问题
* 增强了表单预填充服务的安全性
...
* 程序包管理器中的验证选项用于检测下叠加文件（JSP 或 JavaScript 文件）`/apps`与修补程序中包含的文件之间是否存在任何冲突`/libs`。然后，受影响的叠加可以重新设置为包含对下文件的更改`/libs`。NPR-16216：适用于 CQ-81729 的修补程序
...
* 从 AEM 6.0 升级到 AEM 6.2 后，在搜索标记时，因大量查询而导致经典 UI 性能缓慢。要解决此问题，可执行[在标记控制台中禁用复制状态（Classic UI）](#disable-replication-status-in-tagging-console-classic-ui-npr)中所述的步骤。NPR-15842：适用于 CQ-4201748 的修补程序.

* 在 Touch UI 中创建页面时，“name”字段的输入检查不检查特殊字符“撇号”（与Classic UI 中的情况相同）。因此，无法移动页面。NPR-16404：适用于 CQ-4205321 的修补程序。
* 在富文本编辑器中对两行应用不同样式后将两行合并，会删除应用于第二行的样式。NPR-16389：适用于 CQ-4203835 的修补程序.
* 在 Touch UI Sites 屏幕中，无法尝试将页面粘贴到不含子页面的页面中，因为“粘贴”按钮不可用。NPR-15894：适用于 CQ-4201696 的修补程序.
* 在内容查找器面板中滚动“页面”选项卡时，Classic UI 中会无限显示几组页面，而 Touch UI 则仅显示有限的少数几个非重复页面。NPR-16271：适用于 CQ-4202371 的修补程序
...
* 当在内容层次结构下创建标题包含特殊字符的文件夹时，将显示这些字符的编码版本。NPR-15935：适用于 CQ-4202987 的修补程序
...
* 同步翻译作业现在不会自动触发，并且在未存入翻译项目的情况下，AEM 轮询不会发生。NPR-15163：适用于 CQ-90856 的修补程序
...
* 在“预览”或“编辑”模式下处理信件中的文本片段时，如果将文本转换为列表，则将损坏整个信件功能。并且会产生 JavaScript 错误。NPR-16460
...
* 保护表单预填充服务免遭通过“file://”、“http://,”和“ftp://”之类的协议进行的未经授权访问。有关详细信息，请参阅[使用配置管理器配置预填充服务](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)。NPR-15434
...
* 在 AEM Forms Designer 中，无法将词典拼写检查中的表单区域设置更改为“法语（加拿大）”。NPR-15896
...
* 导航到页面组件时，Classic UI 对话框中不会获取具有 `tagBasePath` 属性的标记的值。NPR-15543：适用于 CQ-4199950 的修补程序
...
* 用户在 CRXDE 中选择一个节点（例如，`/content/dam/`）。然后单击“Access 控制”选项卡，确保 Access 控制列表存在。现在，拖放某些元素会移动元素，而不是选定的元素。NPR-15696：适用于 GRANITE-16300 的修补程序
...
* 在用户单击锚点图标并添加名称后，Touch UI 富文本编辑器中的锚点插件会生成不合规的 HTML5。它应在锚点元素的 HTML5 标记中添加“id”属性，而不是“name”属性。NPR-15650：适用于 CQ-89782 的修补程序
* 当创建具有大量字段的元数据架构并将其应用于内容片段元数据时，内容片段元数据屏幕上不会创建滚动条，从而导致这些字段无法编辑。NPR-15478：适用于 CQ-4202622 的修补程序
* 编辑 `TagInput` 字段组件不会显示以前针对对话框字段配置的值。NPR-15464：适用于 CQ-4200360 的修补程序

* 在内容片段编辑器 UI 中，当创建了内容片段的多个变量时，则侧面板不会显示用于导览所有变量的滚动条。NPR-15445：适用于 CQ-4199444 的修补程序
...
* 经典 UI 中的“幻灯片放映”组件无法创建幻灯片或拖放图像以创建幻灯片。NPR-15281：适用于 CQ-4194164 的修补程序
...
* “Sites”组件的“页面属性”缩略图选项卡上缺少“清除”按钮。NPR-15143：适用于 CQ-4196997 的修补程序
* 对于使用 Live Copy 的站点，在站点 Admin Console 的“列”窗格中选中“Live Copy”复选框不会正确显示 Live Copy 状态，而只显示 HTML 标记。NPR-15108：适用于 CQ-97086 的修补程序
* 编辑内容片段时，如果用户在收到 Post 响应之前单击“完成”（“√”）以完成编辑，则无法正确保存已编辑的内容。NPR-15014：适用于 CQ-4194095 的修补程序
* 在“时间扭曲”模式下关闭“编辑”页面并尝试从 Site admin 重新打开该页面时，会导致出现状态为“500”的错误，而不是重新打开页面`500`。NPR-14965：适用于 CQ-109647 的修补程序
* 在 Digital Asset Manager (DAM) UI 中，“用户选取器查找可授权项”搜索会导致“内存不足”异常。NPR-15307：适用于 CQ-98542 的修补程序

### 资源 {#assets-17}

* 在 Omnisearch 中搜索资源后，选择资源并尝试通过单击&#x200B;**“查看属性”**&#x200B;来编辑属性，然后&#x200B;**“保存”**&#x200B;按钮，会将用户重定向到空白页面。NPR-15900：适用于 CQ-4202372 的修补程序
* 资源用户界面不响应事件。选择资源并单击“发布”或“呈现版本”不会执行任何活动。NPR-15828：适用于 CQ-4202247 的修补程序
* 从卡片视图发布资源时，卡片不会更新以反映已发布状态。相反，页面会被刷新。NPR-15826：适用于 CQ-102732 的修补程序
* 包含资源修补程序的累积修补程序。NPR-15225
* 如果一个资源文件夹的名称中包含与号（“&amp;”）字符，则在导航到资源时，文件夹名称无法正确显示。NPR-15775：适用于 CQ-4201735 的修补程序
...
* 使用 Firefox 浏览器访问“我的链接共享”UI 时，不会显示共享项目或用户，而且屏幕不可用。NPR-15539：适用于 CQ-4200992 的修补程序
* 使用 Digital Asset Manager 时，如果某个页面与一组图像相关联，则将这些图像移动到新文件夹会破坏页面关联，并且关联的页面会缺少一些图像。因此，关联页面缺少一些图像。NPR-15538：适用于 CQ-111479 的修补程序
...
* 在列视图中使用 Asset Finder 控制台时，尝试创建文件夹会失败一次，但再试一次会成功。NPR-15370：适用于 CQ-4199448 的修补程序
...
* 在 data-sly-resource 中使用名称包含圆点或句点 (.) 的资源，不会解析到同一资源，输出路径将在圆点处终止。NPR-15069：适用于 CQ-4195914 的修补程序
* 将 AEM 6.2 升级到服务包 1 后，将资源同步到 Scene7 失败。`dam:Scene7FileStatus` 属性显示 `UploadFailed` 状态，即使对于已发布的资源也是如此。NPR-15269：适用于 CQ-4197708 的修补程序

### 用户界面 {#user-interface-5}

* 在&#x200B;**[!UICONTROL Touch UI]** 中，使用 Internet Chrome 浏览器版本 56.0.2924.87 时，对于没有 type=&#39;datetime&#39; 的日期字段，不会显示保存的日期。NPR-15383：适用于 GRANITE-16481 的修补程序
...
* Omnisearch 不允许客户使用`/apps` 添加列数据源，因为它假定位置配置列在 */libs/granite/omnisearch/content/metadata/ 下*。NPR-13188：适用于 GRANITE-16479 的修补程序
* 使用&#x200B;**[!UICONTROL 触屏 UI]** 时，产品变量的创建级别与产品不同。系统不会通知用户变量创建过程的状态。NPR-15345：适用于 CQ-4198948 的修补程序

**Scene7**

* 运行 Scene7 工作流程导致打开的文件不会关闭。请求改进 AEM-S7 服务，以便通过共享池配置来维护和重用单个 HttpClient 实例。NPR-15357：适用于 CQ-109958 的修补程序

### 翻译 {#translation-9}

* 使用翻译项目时，从英语主版本更新语言副本会产生 11 个单独的启动项，并且所有启动项都具有相同的名称和源根。但是，它们各自的启动根略有不同，以防页面名称遵循既定的模式。NPR-15605：适用于 CQ-4200699 的修补程序
...
* 在“管理资源”中设置的关于包含多个标题的文本制表符对齐方式，在 CCR UI 中损坏。NPR-15818
...
* Sling Distribution 同步从创作到发布的分发包时出现缓存错误。NPR-13034：适用于 GRANITE-13970 的修补程序
...
* 如果创建了多个富文本字段（例如链接），“目标”下拉列表在 Touch UI 对话框中无法使用。NPR-13911
...
* 无法正常显示具有 Repeat with Condition 表达式的资源。当用户预览 `*CDN3835RLCEN*` 信件模板时，未显示位于“正文”目标区域中的资源。对于允许使用多个附件的文件附件构件，如果在含有先前附件的构件上提交了一个具有附件的新表单实例，则会显示错误代码。NPR-14844

* 创建智能收藏集时，在保存智能收藏集后不保留样式标记。NPR-15081：适用于 CQ-4195494 的修补程序
* 在多个用户进行并发搜索期间，资源搜索查询在 Touch UI 中运行缓慢。NPR-15019：适用于 CQ-4195405 的修补程序
...
* 具有复制权限的用户现在可删除资源文件夹。NPR-14321：适用于 CQ-88271 的修补程序
...
* 自 `Jquery` 更新以来，用户无法选择资源收藏集并在内容片段的“关联内容”面板中确认选择。NPR-14847：适用于 CQ-4194209 的补丁包
...
* 启动工作流时，用户无法从“工作流收件箱”控制台预览有效负荷。NPR-13226：适用于 CQ-100275 的修补程序
* 使用 SAML 身份验证处理程序配置的“saml_request_path”cookie 显示带有额外`?`字符的 cookie。此外，当 SAML 响应回发到 AEM 时，AEM“saml_request_path”Cookie 会因字符已编码而返回无效值。NPR-13517：适用于 GRANITE-11722 和 GRANITE-14414 的主动修补程序

### Dynamic Media {#dynamic-media}

* 在 AEM 激活过程中创建和取消的大量 `AEM-Scene7` 作业在复制期间记录为存档作业。NPR-12835：适用于 CQ-86115 的修补程序

### 安全 {#security-5}

* 请求解决 WCMDebug 筛选器中的输入验证问题。NPR-12444：适用于 CQ-94890 的修补程序请求
* 使用“创建启动项”向导时，主动请求更正 XSS 行为。NPR-13062：适用于 CQ-99577 的修补程序请求
...
* 如果您为单选按钮设置了默认值，并在保存或提交期间选择了不同的值，则预填不会显示所选的值。NPR-15304
...
* 将 HTML Workspace 中转发的草稿任务返回给初始用户时，该任务不会显示在初始用户的队列中。NPR-15178
...
>[!NOTE]
>
>Adobe 提供的可选功能包依赖于发行版本和累积修订包。如果您已安装功能包，请联系 [AEM 客户关怀团队](https://experienceleague.adobe.com/?support-solution=General#support)，验证与 AEM 6.2 的累积修订包的兼容性。

>[!NOTE]
>
>我们建议您先对每个新的安装包运行验证，然后再尝试安装。预验证会分析并报告安装之前发现的任何错误，并主动向用户发出有关此类错误、叠加和权限的警告。
>

* AEM 6.2 服务包 1 是安装 CFP 的先决条件。有关安装说明，请参阅 [AEM 6.2 服务包 1](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) 的发行说明。
...
若要在现有 AEM 6.2 SP1 实例上安装累积修订包，请执行以下步骤：

1. 单击 [Software Distribution](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq640/cumulativefixpack/aem-6.4.8-cfp-2.0.zip) 以下载该包。
...
* 使用[包管理器中的 HTTP API](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) – 确保使用 `cmd=install&recursive=true` – 这会安装嵌套包。
...
1. 所有 OSGi 包在 OSGi Console 中均为“活动”或“片段”（使用 Web 控制台：/system/console/bundles）。
...
## DTM、Analytics、Target 连接的用户可配置超时参数 {#user-configurable-timeout-parameters-for-dtm-analytics-target-search-promote-connections}

使用 AEM 累积修订包 6.2 SP1-CFP7 及更高版本时，可根据以下详细信息对上述所有连接配置连接超时时间段：

| **连接** | **连接超时&#42;** | **套接字超时&#42;&#42;** |
|---|---|---|
| DTM | 30000 毫秒 | 30000 毫秒 |
| 分析 | 30000 毫秒 | 30000 毫秒 |
| 目标 | 60000 毫秒 | 30000 毫秒 |

* **连接超时&#42;** - 在建立连接之前的超时时间（以毫秒为单位）。超时值为 0 表示无限超时。
...
* 叠加&#x200B;*“/libs/cq/tagging/widgets/source/widgets/admin/TagAdmin.js”*&#x200B;在&#x200B;*`/apps`*
...
1. 在 `jsafeFIPS.jar`AEM_Forms_Installation_directory `jsafeJCEFIPS.jar`中创建 `certjFIPS.jar`、[和]`/jboss/modules/system/layers/base/com/adobe/livecycle/main/` 文件的备份，然后从前面提到的目录中删除这些文件。
...
当某个用户声明任务时，不会为其他用户刷新共享队列的计数。为此，Adobe 引入了一个新属性。请按照以下步骤在 AEM 实例上配置此属性：

1. 转到“管理员 UI”>“服务”>“工作区”>“全局管理”。
1. 导出全局设置。
1. 在下载的 XML 文件中，添加标签 `<client_tasksPollingInterval>10</client_tasksPollingInterval>`。十是以秒为单位的示例值。您可以相应地对其进行修改。
1. 保存文件。
...
* 对于将 `dc:title` 属性设置为 String []（多字段）的图像，在图像卡片上显示标题时发生行为更改：尽管所有标题都保存在 CRX 中，但 UI 中的图像卡片上只显示最新更改的标题。适用于 CQ-4217165 的修补程序
...
* 打开 Web Console 组件详细信息页面时，收到 HTTP 500 内部服务器错误。
* 由于存储库重新启动而发生诸如&#x200B;**创建组件实例**&#x200B;和&#x200B;**服务工厂返回 null** 之类的错误：
...
>[!MORELIKETHIS]
>
>* [AEM 6.2 修补程序页面](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-release-information/aem-release-updates/aem-releases-updates)
>* [AEM 6.2 SP1 发行说明](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)
>* [AEM 6.2 发行说明](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)
>* [AEM 产品页面](https://business.adobe.com/products/experience-manager/adobe-experience-manager.html)
>* [AEM 6.2 文档](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)
>* [Adobe 优先产品更新](https://experienceleague.adobe.com/zh-hans/docs/release-notes/experience-cloud/current)
