---
title: AEM 6.3 累积修复程序包
description: AEM 6.3 Cumulative Fix Pack发行说明
...
# AEM 6.3累积修复包发行说明{#release-notes-aem-cumulative-fix-pack}
...
|---|---|
| **版本** | 6.3 |
| **发行版** | [PackageShare](https://www.adobeaemcloud.com/content/marketplace/marketplaceProxy.html?packagePath=/content/companies/public/adobe/packages/cq630/cumulativefixpack/AEM-CFP-6.3.3.8)、[软件分发（测试版）](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq630/cumulativefixpack/aem-6.3.3-cfp-8.0.zip)上的累积修补程序包6.3.3.8 |
| **先决条件** | [AEM 6.3 Service Pack 3 (6.3.3.0)](https://helpx.adobe.com/experience-manager/6-3/release-notes/sp3-release-notes.html) |
| **正式发布** | 2020年3月5日 |
...
有关CFP和其他类型版本的详细信息，请参见[维护版本车辆定义。](https://docs.adobe.com/content/docs/en/aem/6-3/deploy/maintenance-release-vehicle-definitions.html)
...
AEM Cumulative Fix Pack 6.3.3.8取决于AEM 6.3 Service Pack 3。 因此，在安装AEM 6.3 Service Pack 3后，必须安装AEM Cumulative Fix Pack 6.3.3.x包。 有关安装说明，请参阅 [AEM 6.3 Service Pack 3 发行说明](https://helpx.adobe.com/experience-manager/6-3/release-notes/sp3-release-notes.html)。
...
>[!NOTE]
>
>对于版本低于6.3.3.0的AEM实例，Adobe建议通过安装文件夹为AEM实例上有大量用户的客户部署SP/CFP。
...
* 在向收藏集添加资产时，“添加到收藏集”页面不会在卡片视图中显示所有可用的收藏集(NPR-32537)。

### 站点 {#sites}

* 当您选择parsys和其中的组件并使用键盘快捷键删除选定的项目时，该操作会同时删除该组件及其父parsys(NPR-32071)。
* 保存页面属性时，会创建一个不正确的节点(NPR-31774)。

### 集成 {#integrations}

* ReportSuitesServlet易受SSRF(NPR-32162)的攻击。

### 活动定位{#campaign-targeting}

* 在创作实例中更改后激活的组件的内容，在发布实例中不可见，直到重新启动&#x200B;**com.day.cq.personalization.impl.TargetedContentManagerImpl**（NPR-32489和NPR-32232）。
* Contexthub在发布时性能崩溃(NPR-31170)。

### Brand Portal {#brand-portal}

* Adobe I/O未与Adobe Experience Manager6.3(NPR-32056)整合，用于品牌门户。
...
* 设计人员：如果启用标记选项，子表单边框将消失在生成的PDF输出中（NPR-32324和NPR-32545）。
* 设计人员：如果表中存在合并的单元格，则使用输出服务从XDP表单转换的输出PDF文件的辅助功能测试将失败(NPR-32068)。
* 文档安全：受保护的PDF文件无法脱机打开，`DisableGlobalOfflineSynchronizationData`选项设置为`True`(NPR-32080)。
...
AEM Cumulative Fix Pack 6.3.3.7取决于AEM 6.3 Service Pack 3。 因此，在安装AEM 6.3 Service Pack 3后，必须安装AEM Cumulative Fix Pack 6.3.3.x包。 有关安装说明，请参阅 [AEM 6.3 Service Pack 3 发行说明](https://helpx.adobe.com/experience-manager/6-3/release-notes/sp3-release-notes.html)。

### 资产 {#assets-1}

* 在从“仅限内容”下拉列表中选择筛选器选项，然后选择移动选项之前选定的资产（在触屏 UI 的列视图中）也会移动 (NPR-30693)。
* 在工作流处理的创作实例中，`${extension}`变量未呈现(NPR-31694)。
...
* 开箱即用的基架富文本编辑器 (RTE) 意外地将内嵌字体大小应用于元素(NPR-31283、NPR-30922)。
* 无法在包含开箱即用的设计导入程序组件的 Adobe Campaign 中同步营销活动 (NPR-30890)。
* 富文本编辑器 (RTE) 不允许将嵌入式表格作为列表项插入 (NPR-30878)。
* 当用户专注于左边栏字段并使用键盘快捷键粘贴内容时，它会粘贴页面编辑器剪贴板的内容，而不是从左边栏字段复制的内容(NPR-31173)。
* 用户编辑内容片段时，会恢复内容片段的已删除变量 (NPR-31272)。
* AEM站点没有创建语言副本的选项(NPR-30690)。
...
* 在添加子表单的实例时，生成 XDP 表单的 HTML5 预览会显示闪烁 (NPR-30908)。
...
AEM Cumulative Fix Pack 6.3.3.6取决于AEM 6.3 Service Pack 3。 因此，在安装AEM 6.3 Service Pack 3后，必须安装AEM Cumulative Fix Pack 6.3.3.x包。 有关安装说明，请参阅 [AEM 6.3 Service Pack 3 发行说明](https://helpx.adobe.com/experience-manager/6-3/release-notes/sp3-release-notes.html)。
...
AEM Cumulative Fix Pack 6.3.3.5取决于AEM 6.3 Service Pack 3。 因此，在安装AEM 6.3 Service Pack 3后，必须安装AEM Cumulative Fix Pack 6.3.3.x包。 有关安装说明，请参阅 [AEM 6.3 Service Pack 3 发行说明](https://helpx.adobe.com/experience-manager/6-3/release-notes/sp3-release-notes.html)。
...
* （体验片段）FacebookApplication#setUpApp使用已弃用的API，它不再有效。 NPR-29213：适用于 CQ-4266630 的修补程序
...
AEM Cumulative Fix Pack 6.3.3.4取决于AEM 6.3 Service Pack 3。 因此，在安装AEM 6.3 Service Pack 3后，必须安装AEM Cumulative Fix Pack 6.3.3.x包。 有关安装说明，请参阅 [AEM 6.3 Service Pack 3 发行说明](https://helpx.adobe.com/experience-manager/6-3/release-notes/sp3-release-notes.html)。
...
* (MSM)使用oak-index实施LiveCopyIndex。 NPR-29198：适用于 CQ-4222472 的修补程序
* “coral.js”文件包含库“handlebars.js”的漏洞版本。NPR-26973：适用于 CQ-4255377 的修补程序
* 使用具有嵌套布局目标和文本组件的容器组件时，在编辑文本或单击容器时，将引发JavaScript错误“无法读取属性currentPos为null”。 NPR-29077：适用于 CQ-4246594 的修补程序
* （触屏UI）无法批量更新标记到已被不同标记标记的页面。 NPR-28729：适用于 CQ-4262922 的修补程序
* 在卡片视图中打开变量会导致 500 错误。NPR-28611：适用于 CQ-4263571 的修补程序
* 转出已在母版中移动的结构会导致错误的 cq:moveTarget。NPR-28968：适用于 CQ-4265280 的修补程序

### 集成  {#integration}

* (Cloud Service配置)应删除在根级别显示的“继承自”复选框。 NPR-28771：适用于 CQ-4259676 的修补程序
...
* PushOnModify PageManager移动的转出多次从两个会话中提交，导致竞争条件。 NPR-28880：适用于 CQ-4266191 的修补程序
...
AEM Cumulative Fix Pack 6.3.3.3取决于AEM 6.3 Service Pack 3。 因此，在安装AEM 6.3 Service Pack 3后，必须安装AEM Cumulative Fix Pack 6.3.3.x包。 有关安装说明，请参阅 [AEM 6.3 Service Pack 3 发行说明](https://helpx.adobe.com/experience-manager/6-3/release-notes/sp3-release-notes.html)。

**AEM 累积修补程序包**&#x200B;的主要功能亮点包括：

* 内置存储库 (Apache Jackrabbit Oak) 已更新至版本 1.6.16。
* 策略集列表分页更改为每页限制 50 条记录。
* 添加了代表：在发布实例上缓存到AEM Communities用户同步监听器上的可忽略节点。
* 添加了列表和卡视图按钮的标签。
* 执行任何搜索时，包含逗号的转义字符。
* 启用了对内容策略的综合资源的支持。
...
* ImageServer发布设置不会复制到混合投放。 NPR-28329：适用于 CQ-4253030 的修补程序

#### 社区  {#communities-4}

* 发布时，为 AEM Communities 启用组件启用了键盘导航。NPR-27739：适用于 CQ-4253856 的修补程序
* 启用了键盘导航来触发内容播放。NPR-27738：适用于 CQ-4254026 的修补程序
* 添加了列表和卡视图按钮的标签。 NPR-27736：适用于 CQ-4254027 的修补程序
* （支持）新增代表：在发布实例上缓存到AEM Communities用户同步监听器上的可忽略节点。 NPR-27841：适用于 CQ-4247234 的修补程序
* 在UI级别的搜索框中，特殊字符的前缀为转义字符(\)。 NPR-27839：适用于 CQ-4259757 的修补程序
* 搜索(, +,?)等字符时出错中。 NPR-28212：适用于 CQ-4260969 的修补程序
...
* 在AEM 6.3中运行版本清除会在日志中不断发出警告。 NPR-27750：适用于 CQ-4206870 的修补程序
* 富文本编辑器全屏模式不支持样式插件。NPR-27622：适用于 CQ-4258674 的修补程序
* 列表“loaderPromies”在editor.js NPR-27768中加载内容框架后未清除：CQ-4205337的修补程序
* 在父组件上未设置则无法在嵌套parsys上设置模板策略。 NPR-27987：适用于 CQ-4246095 的修补程序
* 组件浏览器无法清除用户输入的内容，因此可能会引发 JavaScript 错误。NPR-27986：适用于 CQ-4247590 的修补程序
* 当用户尝试编辑内容片段时，该页面显示为空。 NPR-27669
...
* 在将MS Translator升级到API v3.0后，在AEM 6.3中支持MS Translator服务。NPR-28365:CQ-4259096的修补程序
...
* 引发一个名为java.lang.IllegalArgumentException消息的异常：没有枚举常数com.adobe.internal.pdfm.docbuilder.signature.PathValidationFailureReason.SIGNED_IN_FUTURE阻止短时进程完成。 NPR-28652

#### 表单 - 自适应表单  {#forms-adaptive-forms}

* 在选中复选框项时，下拉列表中的添加／删除项目不会更新。 NPR-28224：适用于 CQ-4252834 的修补程序
...
AEM Cumulative Fix Pack 6.3.3.2取决于AEM 6.3 Service Pack 3。 因此，在安装AEM 6.3 Service Pack 3后，必须安装AEM Cumulative Fix Pack 6.3.3.x包。 有关安装说明，请参阅 AEM 6.3 Service Pack 3 发行说明。
...
* 挂起父页删除cq :缺少页面中的LiveRelationship混合类型。 NPR-26996：适用于 CQ-4254113 的修补程序
* （外部链接检查器）内部链接在单个页面上显示为已断开，但外部链接则不能使用。 NPR-27481：适用于 CQ-4257780 的修补程序
* 编辑其他页面属性时，云服务配置继承中断。NPR-27311：适用于 CQ-4256785 的修补程序
* 将核心组件  并添加一个基础表单。 NPR-27333：适用于 CQ-4249176 的修补程序
* 富文本编辑器删除了空的 alt 标记。NPR-26938：适用于 CQ-4253267 的修补程序
* （经典UI）   选择已更改   监听器。 NPR-27115：适用于 CQ-4237215 的修补程序
...
* 不为目标组件考虑    cq:目标组件不考虑操作。 NPR-27616：适用于 CQ-4257497 的修补程序
...
* 在混合媒体集的查看器预设编辑器中不播放视频。 适用于 CQ-4251442 的修补程序
...
* 翻译预览不适用于we.retail示例内容。 NPR-27170：适用于 CQ-4241179 的修补程序

* 主动式 platform.login 修补程序。NPR-26961
* 保存并关闭页面属性不会在使用 Tomcat 的 AEM WAR 中返回  aem WAR中与Tomcat的正确页面。 NPR-27567：适用于 Granite-23671 的修补程序
...
AEM Cumulative Fix Pack 6.3.3.1取决于AEM 6.3 Service Pack 3。 因此，在安装AEM 6.3 Service Pack 3后，必须安装AEM Cumulative Fix Pack 6.3.3.x包。 有关安装说明，请参阅 [AEM 6.3 Service Pack 3 发行说明](https://helpx.adobe.com/experience-manager/6-3/release-notes/sp3-release-notes.html)。
...
* （全名搜索）在集合中搜索时的性能问题。 NPR-26714：适用于 CQ-4253408 的修补程序
...
* 禁用Dynamic Media云配置UI中的“公司根文件夹路径”。 NPR-26361：适用于 CQ-4249505 的修补程序
...
* (DM Hybrid)AEM 6.3.2与Dynamic Media的目录复制中断。 NPR-26406：适用于 CQ-4251306 的修补程序
...
* （触屏UI）当从全屏切换到浮动对话框时，多个富文本编辑器的工具栏变得不可用。 NPR-25652：适用于 CQ-4206008 的修补程序
* 提升多页启动项会为每个页面创建多个版本。NPR-26810：适用于 CQ-4254663 的修补程序
* 标记移动操作在  结构化内容片段模型标记字段。 NPR-26801：适用于 CQ-4251805 的修补程序
* （触屏UI）在Blueprint中重命名页面后，从页面编辑器中取消发布子页面不起作用。 NPR-26774：适用于 CQ-4254175 的修补程序
* 已取消发布的页面无法使用引用。NPR-26749：适用于 CQ-4254372 的修补程序
* 创建Live Copy版本时，用户需要刷新页面以反映。 NPR-26663：适用于 CQ-4254328 的修补程序
* （经典UI）返回页面属性后，缩略图图像不再使用继承，从站点管理员和Sidekick中消失，并且图像显示为空。 NPR-26562：适用于 CQ-4252346 的修补程序
* 创建页面版本并触发比较后，/content/ 版本Blueprint的Live Copy列表中列出了版本Shistory。 NPR-26506：适用于 CQ-4243957 的修补程序
...
* （安全漏洞）由于JWT配置错误而进行帐户接管。 NPR-26440：适用于 CQ-4253314 的修补程序
...
* （论坛搜索）“返回”按钮将跳过页面并传回forum.html。 NPR-26949：适用于 CQ-4254804 的修补程序
* 未读通知不会增加超过 21。NPR-26947：适用于 CQ-4251251 的修补程序
* （SearchScheduledPosts作业）在ConfigMgr中添加启用／禁用切换。 NPR-26924：适用于 CQ-4250463 的修补程序
...
* （论坛搜索）-指向帖子数较多的线程的分页链接不适用于通知。 适用于 CQ-4254202 的修补程序
* 审核控制台在 Firefox 中仅显示 5 个条目。适用于 CQ-4254128 的修补程序
* 创建站点时，组不可见。NPR-27148：适用于 CQ-4251788 的修补程序
* 将组和成员添加到“角色”设置并允许博客函数中的特权成员时出现空指针异常。 NPR-21732、NPR-27176：适用于 CQ-4255909 的修补程序
* 编辑站点并在角色设置中添加成员会引发空指针异常。NPR-27132：适用于 CQ-4255909 的修补程序

#### 用户界面 {#user-interface}

* 主动式平台登录修补程序。NPR-26961
* （多字段）允许合成项目具有自定义名称。 NPR-26493：适用于 Granite-20568 的修补程序
...
* (IE11)在键入负值时，数字字段中显示“NaN”。 NPR-26701：适用于 CQ-100826 的修补程序
* (珊瑚。 多字段)嵌套的多字段使用错误的模板创建项目。 NPR-25649：适用于 CUI-6743 的修补程序
* 更新花岗岩coralui2和coralui3 clientlibs以从生成中删除Handlebars。 NPR-25606：适用于 Granite-22116 的修补程序
...
* 从管理员UI设置搜索模板，然后使用相同模板在AEM Forms工作区中搜索任务会引发异常。 适用于 CQ-4255649 的修补程序
...
* 运行LCM时，adobe.livecycle.weblogic.ear和adobe.livecycle.websphere.ear的部署失败。 适用于 CQ-4255629、CQ-4255630 的修补程序
* 应用程序管理中的错误消息无效。 NPR-23289：适用于 CQ-4233163/CQ-4255636 的修补程序
...
AEM Cumulative Fix Pack 6.3.2.2取决于AEM 6.3 Service Pack 2。 因此，在安装AEM 6.3 Service Pack 2后，必须安装AEM Cumulative Fix Pack 6.3.2.x软件包。 有关安装说明，请参阅 [AEM 6.3 Service Pack 2 发行说明](https://helpx.adobe.com/experience-manager/6-3/release-notes/sp2-release-notes.html)。
...
* 如果expressionResolver为null，则确保的渲染条件返回一个正常结果。
...
* (ExpiryNotificationJobImpl)优化现有查询。 NPR-23330：适用于 CQ-4205249 的修补程序
* (元数据用户档案)创建时设置的标准标记值在保存后不可用。 NPR-23370：适用于 CQ-4235458 的修补程序
* （触屏UI）由于JS错误，无法移动多个资产。 NPR-23395：适用于 CQ-4241279 的修补程序
...
* 页面属性中的缩略图选项卡在扩展对话框时不起作用。 NPR-22844：适用于 CQ-4241474 的修补程序
...
* （页面编辑器）再次打开对话框时，图像组件上的图像会消失。 NPR-23156：适用于 CQ-4239978 的修补程序
* 滚动到页面底部时，模板编辑器仅显示 20 个模板/文件夹，而不会加载其他模板/文件夹。NPR-23185：适用于 CQ-4238483 的修补程序
* （经典UI）移动或重命名页面时会生成错误。 NPR-23213：适用于 CQ-4240971 的修补程序
...
* （富文本编辑器）工具提示显示插件的变量名称而不是可读插件名称。 NPR-23550：适用于 CQ-4243269 的修补程序
...
* (目标)在将API类型用作XML时，活动不会激活。 NPR-23199：适用于 CQ-4240936 的修补程序****
...
* (ContextPath)在社区组成员列表中搜索组成员时，中断Jetty配置的消息功能会中断，并出现错误。 NPR-23336：适用于 CQ-4241519/CQ-4242080 的修补程序
...
* 在论坛组件上执行  在论坛组件上搜索。 NPR-23838：适用于 CQ-4243770 的修补程序
* (Communities Moderation Parking)标记消息的批量允许无效。 NPR-23845：适用于 CQ-4243962 的修补程序
* 尽管选择了默认排序，但排序按钮文本不显示默认  价值   的双曲余切值。 NPR-23881：适用于 CQ-4243375 的修补程序
...
* 滚出页面时，Live Copy不显示从主控继承的开启／结束时间属性。 NPR-23873：适用于 CQ-4243431 的修补程序
* LiveCopy 操作不会忽略已删除组件的子节点。NPR-23058：适用于 CQ-4211662 的修补程序

### 卸载 {#offloading}

* 请求机制在处理后或定期从工作人员实例中清理资产。 NPR-23638：适用于 Granite-21337 的修补程序
...
* (FDM)(WebService)支持WSDL分析器中WSDL的扩展构造。 NPR-23640、NPR-23236：适用于 4205821 的修补程序
...
* （触屏UI）列视图中资产的默认上传位置。 NPR-22465：适用于 CQ-4237057 的修补程序
...
* (DAM)添加到集合向导时存在多个跨站点脚本(XSS)漏洞。 NPR-22432：适用于 CQ-4327086 的修补程序
...
* Workflow收件箱中的新项目显示页面路径而非页面标题。 NPR-21634：适用于 CQ-4230672 的修补程序
* 可编辑的结构组件在编辑后丢失响应式网格所需的 CSS 类名称。NPR-21741：适用于 CQ-4232374 的修补程序
* (TouchUI)HTL组件存在多个跨站点脚本(XSS)漏洞。 NPR-21899：适用于 CQ-4232511 的修补程序
* 无法更改内容片段混合媒体图像资源类型。NPR-21907：适用于 CQ-4233401 的修补程序
* 触屏 UI 对话框的 RTE 全屏会隐藏 RTE 插件。NPR-22034：适用于 CQ-4235457 的修补程序
* （触屏UI）富文本编辑器从&lt;a>标记中删除除id之外的所有属性。 NPR-22044：适用于 CQ-4234133 的修补程序
...
* （经典UI）富文本编辑器输出html添加了“列表位置样式：inside;&#39;作为&lt;ul>标记的内联样式。 NPR-22145：适用于 CRTE-114 的修补程序
...
* （经典UI）粘贴文本快捷键(Ctrl+V)对OOTB文本（富文本）组件无效。 NPR-22224：适用于 CQ-4236224 的修补程序
* 键入文本时，无法按预期执行 Tagfield 筛选。NPR-22236：适用于 CQ-4236655 的修补程序
* （页面编辑器）在图像映射组件中粘贴文本数据时，在图像映射组件中粘贴文本数据时，文本组件也会被粘贴。 NPR-22264：适用于 CQ-4236230 的修补程序
...
* 如果在  第一个内容根目录。 NPR-22482：适用于 CQ-4237818 的修补程序
* [触屏 UI] 通过经典 UI 控制台删除启动项会使所有页面无法编辑。NPR-22491：适用于 CQ-4225074 的修补程序
* 由于对话框时，工具栏将无法在多个富文本编辑器中使用。NPR-22528：适用于 CQ-4238183 的修补程序
* 使用   在内嵌模式下，以前加载的插件在第二次时不可见。 NPR-22591：适用于 CQ-4236850 的修补程序
* 删除嵌套启动项中的启动项，会导致子启动项变得孤立。NPR-22621：适用于 CQ-4202639 的修补程序
* （经典UI Sidekick）当页面处于工作流锁定阶段时，将禁用“工作流”选项卡。 NPR-22722：适用于 CQ-4237557 的修补程序
* 在页面上翻转图像组件中添加的图像后，更改不会保存，并且  原始图像显示在页面上。 已通过[https://github.com/Adobe-Marketing-Cloud/aem-core-wcm-components/pull/141](https://github.com/Adobe-Marketing-Cloud/aem-core-wcm-components/pull/141)向图像核心组件添加了渲染支持。 NPR-22801：适用于 CQ-4221539 的修补程序
...
* 带有 OS 剪贴板以及内部 AEM 剪贴板的触屏 UI 中存在复制/粘贴问题。NPR-22807：适用于 CQ-4220383 的修补程序
* Lucene 搜索返回的“摘录”高亮显示内容不一致。NPR-22879：适用于 CQ-4238513 的修补程序
* 在关闭  发布实例时，会生成绿色状态，而不是变为琥珀色。 NPR-22927：适用于 CQ-4236310 的修补程序
* (StyleSystem)从弹出窗口中选择样式时屏幕位置会跳转。 NPR-23183：适用于 CQ-4238867 的修补程序
* （管理发布）要移动到日历的下个月，需要多次单击。 NPR-23508：适用于 CQ-4242732 的修补程序
...
* 由于缺少   jcr:内容节点。 NPR-22850：适用于 CQ-4222122 的修补程序
* 使用  非英语。 NPR-22917：适用于 CQ-4218213 的修补程序
* 发布具有目标内容的页面会丢失相关资源。NPR-23064：适用于 CQ-4227119 的修补程序
* 用户看不到   在云配置中将AT.js作为客户端库进行测试时可看到的mbox调用。 NPR-21930：适用于 CQ-4234520 的修补程序
...
* CAPTCHA组件已弃用以提高安全性，如果您使用的是CAPTCHA组件，则会显示消息“Captcha组件已弃用，不应再使用。” 消息。AEM组件可以进行自定义以包含reCAPTCHA以增强安全性NPR-22151:CQ-4220052的修补程序
...
* （经典UI）组件显示默认值，即使关联的表单数据模型服务设置为空字段也是如此。 NPR-22272：适用于 GRANITE-19744 的修补程序

### 花岗岩  {#granite-2}

* 内存泄漏导致的Asset Share AEM发布器实例的稳定性问题。 NPR-22205、NPR-23178：适用于 Sling-5668、Sling-7292 和 Sling-7470 的修补程序。
* 不稳定的服务 ID 不应当用于会话属性名称。NPR-22821：适用于 Granite-21059 的修补程序
* 当  http   白板管理的会话失效，如果会话没有其他会话属性，容器会话也失效。 NPR-23059：适用于 FELIX-5819 的修补程序
...
* （多语言项目）项目不列表19种语言的翻译作业。 NPR-22498：适用于 CQ-4229978 的修补程序

### 工作流 {#workflow}

* （经典UI）启用或禁用工作流启动器会导致错误行为。 NPR-22907：适用于 CQ-4239153 的修补程序
...
* 规则编辑器的自定义功能正在添加额外功能；因此，即使自定义函数返回true，验证也会失败。 NPR-22481：适用于 CQ-4235499 的修补程序
...
* (Microsoft Edge)(IE11)具有多行字段的自适应表单文本字段组件将“Null”显示为默认值，而不是空。 NPR-22284：适用于 CQ-69107 的修补程序
...
* (Firefox)无法更新自适应Forms的XML模式属性，因为记录文档(DOR)中的选项未在属性页中作为预选。 NPR-22298：适用于 CQ-4237402 的修补程序
...
* AcrobatDC与AEM之间的差异报告了PDF/A-1b规范检查错误。 NPR-22051、NPR-22050：适用于 CQ-4226128、CQ-4227671 的修补程序
...
* （HTML工作区）滚动条的屏幕大小问题。 NPR-23288
* （HTML工作区）流程起点不按字母数字顺序排序。 NPR-22841：适用于 CQ-4238944 的修补程序
* （HTML工作区）在工作区中关闭表单时会触发准备数据。 NPR-21127：适用于 CQ-4224574 的修补程序
* （HTML工作区）当调用需要具有详细说明的附注的流程时，“附注”展开按钮不起作用。 适用于 CQ-4241488 的修补程序
...
* 在安装程序和 LCM 中将 Jsafe Jars 更新为 Cryptoj 6.1.3.1。NPR-21370
...
* 包含不兼容UTF-8名称的资源的压缩文件下载不正确。 NPR-20420：适用于 CQ-4219961 的修补程序
* OmniSearch 中的字符过多导致 AEM 服务器崩溃。NPR-20434：适用于 CQ-4223602 的修补程序
* DAM资产API元数据缺陷会中断xmp回写API。 NPR-20607：适用于 CQ-4220455 的修补程序
...
* 已添加apache公用`{org.apache.commons/commons-email/1.5}`的电子邮件捆绑，替换`{com.day.commons.osgi.wrapper/com.day.commons.osgi.wrapper.commons-email/1.2.0-0002}`。
...
* 将Analytics现有凭据迁移到WSSE身份验证。 NPR-19962：适用于 CQ-4218071 的修补程序
...
* 在工作流模型的“对话框”参加者步骤中单击下拉框时，OOTB花岗岩用户选取器不加载任何用户。 NPR-20236
...
* 管理工作流提交列表，仅限十个应用程序。 CQ-4230193

#### HTML5 表单  {#html-forms-3}

* 当辅助功能配置中未定义XDP表单对象（如子表单）时，其名称将作为其工具提示显示。 NPR-20523

### Forms JEE 安装程序  {#forms-jee-installer-9}

#### 进程管理 {#process-management-2}

* FormSetPrefillApp起始点不预填AEM Forms应用程序中的表单集字段。 NPR-20950

#### 表单 - AEM (LiveCycle)  {#forms-aem-livecycle}

* 安装最新的CTJPEG2K库，以解决严重的安全漏洞。 这会影响XMLFM(AEM和IfBA)、RM、PDFG模块。 NPR-20625：NPR-21337。
...
[获](assets/do-not-localize/6_3_1_2-content-package-list.txt)
取文件AEM Cumulative Fix Pack 6.3.1.1是自AEM 6.3 Service Pack 1(6.3.1.0)于2017年10月正式上市以来包含多个内部和客户修复的重要更新。
...
* dc: title 和 dc: description 在   crx /de。 NPR-19570：适用于 CQ-4209086 的修补程序
...
* 当表单最大化／最小化并另存为草稿或转发时，工作流附件、备注和详细信息选项卡在工作区中不起作用。 NPR-20243
...
* 在LiveCopy中重新应用继承时，修改的本地化组件未恢复到其原始表单。 NPR-18172：适用于 CQ-4211379 的修补程序
* 在触屏 UI 的列视图中浏览内容繁多的页面时出现问题。NPR-17799：适用于 CQ-4199611 的修补程序
* `com.day.cq.wcm.core.impl.VersionManagerImpl`中未关闭的资源解析程序。 NPR-17789：适用于 CQ-4211152 的 CFP
...
* 以下对象中的资源解析程序未关闭：
   ...
   * `com.day.crx.delite.impl.AuthHttpContext` NPR-17493:用于GRANITE-17404的CFP
...
AEM Forms 修补程序通过随发行版一起提供的 Forms 附加组件包和其他补丁安装程序进行交付。有关详细信息，请参阅[AEM Forms版](aem-forms-releases.md)。
...
AEM Cumulative Fix Pack 6.3.0.1是自2017年4月AEM 6.3正式上市以来的一次重要更新，包含多个内部和客户修复。AEM Cumulative Fix Pack的主要亮点是：
...
* 在包管理器中提供用于检测叠加和CFP之间冲突的验证选项

## 通过软件分发下载CFP的说明{#download-instructions-for-cfp-via-package-share}

您可以直接从[软件分发](https://www.adobeaemcloud.com/content/marketplace/marketplaceProxy.html?packagePath=/content/companies/public/adobe/packages/cq630/cumulativefixpack/AEM-CFP-6.3.3.8)下载CFP包，或执行以下步骤：
...
>[!NOTE]
>
>我们建议您先对每个新的安装包运行验证，然后再尝试安装包。预验证会分析并报告安装之前发现的任何错误，并主动向用户发出有关此类错误的警告。
>
>可访问位于[https://docs.adobe.com/content/docs/en/aem/6-3/administer/content/package-manager.html#Package%20Validator](https://docs.adobe.com/content/docs/en/aem/6-3/administer/content/package-manager.html#Package%20Validator)的“验证”选项文档

* AEM 6.3.3.0 是安装 CFP 的先决条件。请访问[升级文档](https://docs.adobe.com/docs/cn/aem/6-3/deploy/upgrade.html)以了解有关将AEM安装升级到AEM 6.3的详细说明。
* 对于使用RDBMK或MongoDB的群集部署，CFP包可以安装在使用包管理器的任何作者实例上。
...
* 可以在默认位置[http://localhost:4502/system/console/slinglog](http://localhost:4502/system/console/slinglog)添加新记录器，其属性如下：

   * 日志级别：调试
   * 累加：false
   * 日志文件：logs/activity.log
   * 记录器：org.apache.jackrabbit.vault.packaging.impl.ActivityLog

将在以下位置创建活动.log:   crx -quickstart /logs文件夹。

### 通过软件分发安装累积修补程序包{#install-the-cumulative-fix-pack-via-package-share}
...
* 在服务器运行时将包放入`../crx-quickstart/install`中。 包会自动进行安装。
* 使用包管理器](https://helpx.adobe.com/experience-manager/6-3/sites/administering/using/package-manager.html)中的[ HTTP API —— 确保使用`cmd=install&recursive=true` —— 以便安装嵌套包。

### 验证安装 {#validate-installation}

1. “产品信息”页(`/system/console/  productinfo`)现在应在“已安装产品”下显示更新版本字符串“Adobe Experience Manager，版本6.3.3.8”。
1. 所有OSGI捆绑包在OSGI控制台(使用Web控制台：`/system/console/bundles`)。

>[!NOTE]
>
>成功安装包时，错误日志上会显示一条信息性消息，指示内容包已成功安装，如“Content Package **AEM-6.3.3-Cumulative-Fix-Pack-7**&#x200B;已成功安装。”

>[!NOTE]
>
>自 AEM 6.3.3.1 起，对于大多数消息，Jackrabbit FileVault 中的日志级别已从“信息”更改为“调试”。要获取安装在AEM 6.3.3.1或更高版本上的内容包的安装日志，请为org.apache.jackrabbit.vault.packaging.impl启用DEBUG日志级别。
...
1. 按照[安装AEM forms加载项包](https://helpx.adobe.com/experience-manager/6-3/forms/using/installing-configuring-aem-forms-osgi.html)中的说明安装Forms加载项包。
...
1. 要安装更新，请运行Designer 6.2.0_&lt;Language>_Cumulative_QF.msp文件。
...
>[!NOTE]
>
>如果要安装 6.3.3.0 或更高版本，请执行以下步骤来配置 JBoss 应用程序服务器的设置。如果是在 Oracle WebLogic 或 IBM WebSpehere 应用程序服务器上运行的 AEM Forms 服务器上安装 6.3.3.0，则无需其他配置。有关详细信息，请参阅[AEM 6.3.3.0发行说明](https://helpx.adobe.com/experience-manager/6-3/release-notes/sp3-release-notes.html)。
...
* 安装AEM CFP 6.3.3.x时可能会发生以下错误和警告，并且可以安全地忽略它们：

   * *WARN* [OsgiInstallerImpl] org.apache.jackrabbit.vault.packaging.impl.InstallHookProcessorImpl挂接/META-INF/vault/hooks/cloudservices-wfchangeinstallhook-0.0.2-jar-with-dependependependences.jar抛出运行时异常。
   * *ERROR* [OsgiInstallerImpl] com.adobe.cq.social.cq-social-jcr-provider [com.adobe.cq.social.provider.jcr.impl.SpiSocialJcrResourceProviderImpl(2174)]等待注册更改为完全取消注册的超时。 CQ-4209974.
   ...
   * *ERROR* [FelixDispatchQueue] com.day.cq.dam.cq-dam-core FrameworkEvent ERROR(org.osgi.framework.ServiceException:服务工厂返回null。 （组件：com.day.cq.dam.handler.standard.ps.PostScriptHandler）)
...
>[!NOTE]
>
>此配置设置适用于 CFP 6.3.3.2 及更高版本。它指示您更新`sling` .properties文件中的引导委托属性。

要手动更新adobe- livecycle - cq -author.ear/ cq .war中的更改，请遵循以下步骤：

* 停止 AEM 服务器。
* 转到 adobe-livecycle-cq-author.ear/cq.war
* 打开 adobe-livecycle-cq-author.ear/cq.war/WEB-INF/web.xml 进行编辑：

   * 将&#x200B;**sling.bootdelegation.ibm** param-name值更新为：
...
* 从websphere应用程序服务器卸载以前的企业存档(EAR)文件，然后按照[https://helpx.adobe.com/pdf/aem-forms/6-3/install-single-server-websphere.pdf](https://helpx.adobe.com/pdf/aem-forms/6-3/install-single-server-websphere.pdf)第10.2节中的步骤安装更新的EAR文件
...
>[!NOTE]
>
>此配置设置适用于 6.3.2.2 及更高版本。它指示您通过CRX-DE手动更新访问控制列表(ACL)策略，因为由于“merge_preserve” acHandling,ACL无法通过CFP安装进行更新。
...
该属性控制页面` /  jcr   :content`节点下节点子树的最大深度，直到该深度，存储库中存在的节点将用于获取页面属性。 此属性中指定的深度以下存在的任何节点都将被忽略。

默认值为 1。此值可被  通过覆盖文件constants.js(`/libs/cq/ui/widgets/source/constants.js`)，取消对CQ.PAGE_PROPERTIES_MAX_RECURSION_LEVEL属性的注释，并为其指定所需值（页面的/ jcr:content下的最大深度，直到存储页面属性的数据为止）。

**如果用户需要创建多个页面变体，使页面/ jcr:content节点下的节点数大于1000，请使用以下步骤进行配置更改：**

* 配置 Apache Sling 的属性 JSON MaxResults
* 使用`/system/console/  configMgr`获取Servlet
* 将其值设置为大于1000的数字（这是当前默认值），以使此数字大于/ jcr:content子树中的节点总数，直到上面配置的深度为止。

这使slingGETservlet能够返回所有必需的节点。

## Uber Jar {#uber-jar}

适用于6.3.3.8的Uber Jar位于[Adobe公共Maven存储库](https://repo.adobe.com/nexus/content/groups/public/com/adobe/aem/uber-jar/6.3.3.8/)。

要在Maven项目中使用Uber Jar，请参阅文章[如何使用Uber jar](https://helpx.adobe.com/experience-manager/6-3/sites/developing/using/ht-projects-maven.html)，并在您的项目POM中包含以下依赖项：
...
| 区域 | 功能 | 替换 | 版本 |
|----|-----|-----|-----|
| 资产和 Creative Cloud 集成 | [AEM到Creative Cloud文件](https://helpx.adobe.com/experience-manager/6-3/sites/administering/using/creative-cloud.html) 夹共享在AEM 6.2中引入，以便让创意用户能够访问AEM中的资产。在 Creative Cloud 应用程序中发布的新功能“Adobe 资产链接”提供了更佳的用户体验，能够直接从 Photoshop、InDesign 和 Illustrator 中轻松访问 AEM Assets。<br />Adobe 不会再进一步增强文件夹共享功能。虽然该功能包含在 AEM 中，但强烈建议客户使用替换解决方案。 | Adobe 资产链接或桌面应用程序。有关更多信息，请参阅 [AEM Creative Cloud 集成](https://helpx.adobe.com/experience-manager/6-3/assets/using/aem-cc-integration-best-practices.html)一文。 | AEM 6.3.3.x |
...
>* [AEM 6.3 文档](https://docs.adobe.com/content/docs/en/aem/6-3.html)
