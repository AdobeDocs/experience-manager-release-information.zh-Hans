...
* DatePicker不支援手動設定隱藏欄位所設定的外部型別提示。 變更型別提示會出現轉換錯誤。 NPR-23333：适用于 Granite-21194 的修补程序
...
* 當一個PDF資產有多個相關聯的關鍵字時，PDF關鍵字中繼資料無法正確擷取和錯誤修改。 為解決此問題，已移除PDF資產的「主旨」欄位中繼資料屬性。 不過，您可以編輯中繼資料結構，為「主旨」欄位新增多值文字欄位。 NPR-19126
...
* 當文字模組已儲存，且文字模組中的運算式不包含開頭或結尾運算式標籤時，未顯示錯誤訊息。 文字模組顯示錯誤訊息，且無法在信函中轉譯。 NPR-17798
* 在安装附加组件包时，日志中出现意外错误。NPR-18295

**表单管理器**

* AEM Forms UI會以最舊的資產第一順序列出所有資產。 使用者無法以最新的順序將資產重新排序。 NPR-18451
...
* AEM Forms處理序未填入TaskContext變數。 CQ-4211857
